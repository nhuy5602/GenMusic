...
### 4.23 Triển khai attention-matrix distillation (TinyBERT-style) — thêm bên cạnh velocity distillation

Dù §4.22 kết luận nguyên nhân gốc nhiều khả năng là quy mô dữ liệu (không phải bug), attention-
matrix distillation vẫn được triển khai theo đề xuất — đây là một lever bổ sung hợp lý (khớp
"cách suy nghĩ" của teacher ở tầng attention, không chỉ output cuối) và đã có cơ sở khả thi rõ
từ đánh giá chuyển-trọng-số (§5.3 mục 1): shape ma trận attention không phụ thuộc head_dim như
trọng số Q/K thô, nên né được đúng vấn đề RoPE/head_dim đã chặn hướng chuyển trọng số.

**Thiết kế**: lấy source code thật của `DiT.forward` (DiffRhythm2) qua GitHub — xác nhận nó đã
hỗ trợ sẵn `output_attentions=True`, trả về list attention weights theo layer, không cần hook.
Phía student, `LlamaDecoderLayer.forward` (bản `transformers` đang dùng) âm thầm bỏ attention
weights trước khi return (`hidden_states, _ = self.self_attn(...)`) nên phải hook trực tiếp vào
`self_attn`. Vì layout chuỗi 2 bên khác nhau (teacher: `[Text, Clean, Noisy]` 3 đoạn; student:
`[Text, Noisy]` 2 đoạn — student không có nhánh "clean latent" tương đương), chỉ so sánh đúng
block tự-chú-ý trên phần audio/noisy-frame của mỗi bên, trung bình theo head (né lệch head_dim
teacher=128 vs student=64), resize song song bằng nội suy song tuyến tính để khớp độ dài chuỗi,
và ánh xạ layer theo stride đều (student layer *i* ↔ teacher layer tương ứng, kiểu TinyBERT) vì
teacher có 16 layer còn student chỉ 4-8. Thêm `--beta-attention` (mặc định 0.0, không đổi hành
vi cũ) xuyên suốt `cli.py` → `distill_training.py` → `run_kaggle_distill.py`. Thêm luôn
`--max-records` cho `train-distill` (trước đó chỉ `train-self` có) để làm được smoke test rẻ
(2 bài, 1 epoch) trước khi tin bất kỳ thay đổi lớn nào — đúng tinh thần "kiểm tra rẻ trước khi
tốn quota" đã áp dụng suốt report này.

**Hai bug thật phát hiện qua đúng 1 vòng smoke-test → fix → smoke-test lại (2 bài/1 epoch mỗi
lần, chi phí rất nhỏ)**:
1. **`loss_attention` luôn `None`** trên lần chạy thật đầu tiên (teacher_raw có shape thật
   `(2,16,89,89)`, nhưng **student_raw luôn `None`** ở cả 4 layer) — dù đã set
   `llama_config._attn_implementation = "eager"` (bắt buộc để có attention weights thật, vì
   `sdpa` luôn trả `None` — xác nhận qua đọc trực tiếp source
   `sdpa_attention_forward`/`eager_attention_forward` của `transformers`). Test local (CPU,
   `FakeTeacherWithAttention`) chạy đúng ngay từ đầu — chỉ fail trên GPU Kaggle thật, không có
   GPU local để đào sâu thêm nguyên nhân chính xác (khả năng cao: cách resolve
   `_attn_implementation` khác nhau giữa CPU/CUDA ở đúng bản `transformers` đó). **Fix (không
   phụ thuộc vào việc "eager" có thật sự được chọn hay không)**: bỏ hẳn việc đọc attention
   weights từ giá trị self_attn tự trả về — hook tính lại attention trực tiếp từ chính
   `q_proj`/`k_proj`/rotary embedding của module (đúng công thức `eager_attention_forward`),
   đảm bảo đúng bất kể implementation nào được chọn ngầm bên trong.
2. **`AttributeError: 'LlamaAttention' object has no attribute 'scaling'`** ở lần smoke-test
   tiếp theo (sau fix #1) — bản `transformers` cài trên Kaggle (qua `requirements.txt` của
   DiffRhythm2) khác bản local, không có attribute `.scaling` (có ở bản local dùng để verify).
   **Fix**: tính trực tiếp `head_dim ** -0.5` (hằng số chuẩn của attention scaling) thay vì đọc
   một attribute có tên/tồn tại phụ thuộc phiên bản.

**Xác nhận cuối**: sau 2 fix trên, smoke-test thứ 3 (cùng 2 bài/1 epoch) chạy thành công,
`loss_attention=0.00128` — giá trị thật, không còn `None`, không còn cảnh báo nào từ layer-map.
Test suite local giữ nguyên 20/20 pass qua toàn bộ quá trình.

**Bài học phương pháp**: cả 2 bug đều chỉ lộ ra khi chạy trên GPU Kaggle thật với teacher thật —
test local (CPU, teacher giả) pass ngay từ đầu cho fix #1, và không hề chạm tới code path gây ra
bug #2 (vì `FakeTeacherWithAttention` không dùng `LlamaAttention` thật). Đây là giới hạn thật
của test local với fake teacher: verify được luồng dữ liệu/gradient đúng hình dạng, nhưng không
verify được các chi tiết phụ thuộc phiên bản/hardware của chính `transformers` khi chạy với
teacher/kiến trúc thật — 2-3 lần smoke-test rẻ (2 bài/1 epoch, ~7 phút mỗi lần) trên Kaggle thật
vẫn là bước cần thiết trước khi tin một thay đổi lớn, dù đã test local pass.

**Chưa làm**: chưa chạy thật ở quy mô lớn (250 bài) với `beta_attention > 0` để biết attention-
distillation có cải thiện chất lượng nghe thật hay không — §4.22 đã kết luận đây khó là lever
...
### 5.3 Bốn hướng mới được đề xuất sau §4.22 — chưa thử, cần đánh giá/thực hiện ở phiên có quota mới

Sau khi xác định nguyên nhân gốc là quy mô (§4.22), bốn hướng cụ thể được đề xuất và đánh giá
sơ bộ trong lúc chờ quota mới — tất cả đều nhằm giải quyết đúng vấn đề quy mô đó, theo các
cách khác nhau, thay vì tiếp tục dò hyperparameter ở 250 bài:

1. **Chuyển trọng số teacher→student bằng chiếu toán học (pruning + SVD, không train)** — ý
   tưởng: cho student một điểm khởi đầu đã "biết nhạc" thay vì random init, giống kỹ thuật
   structured-pruning-rồi-tiếp-tục-train (ví dụ Sheared LLaMA) trong literature LLM. Đã kiểm
   tra khả thi bằng cách lấy trực tiếp source code thật của teacher từ GitHub (không suy đoán):
   - **Tin tốt**: `LlamaNARDecoderLayer` của teacher **là subclass thẳng của `LlamaDecoderLayer`
     chuẩn HuggingFace** — cùng bộ ma trận (`q/k/v/o_proj`, `gate/up/down_proj`, RMSNorm), cùng
     công thức `LlamaConfig` mà student dùng. Không phải 2 kiến trúc khác biệt như lo ngại ban
     đầu — tương thích thật ở mức "họ hàng cấu trúc".
   - **Điểm khó thật**: `head_dim` khác nhau (teacher=128 từ 2048/16 head; student=64 ở cả 3
     cấu hình đã thử 256/4, 384/6, 512/8) — vì RoPE tính góc quay theo head_dim, copy/cắt thô
     trọng số attention sẽ lệch pha quay. Nhưng **chỉ `q_proj`/`k_proj` bị ảnh hưởng** (RoPE chỉ
     áp dụng lên Q/K) — `v_proj`, `o_proj`, `gate/up/down_proj`, các lớp norm **không** bị vấn
     đề này, chiếu chiều bằng SVD cho chúng là bài toán chuẩn đã hiểu rõ.
   - **Đánh giá**: khả thi để thử nhưng không phải việc 1-2 giờ — cỡ vài ngày công, vì phần Q/K
     cần một cách chiếu giữ được cấu trúc RoPE (chưa có công thức có sẵn, đây là rủi ro nghiên
     cứu thật). Ngay cả Sheared LLaMA cũng luôn train tiếp sau khi prune — nên kỳ vọng thực tế
     là "khởi tạo tốt hơn random", không phải "toán học thuần, zero-training" hoàn toàn.
2. **LoRA/adapter fine-tune trực tiếp lên teacher (mượn kỹ thuật voice-cloning/TTS hiện đại,
   ví dụ cách VALL-E/Bark thích nghi sang giọng/ngôn ngữ mới với ít dữ liệu)** — thay vì thu
   nhỏ model, giữ nguyên teacher (đã CHỨNG MINH sinh audio có cấu trúc, §4.22) và chỉ train một
   số ít tham số adapter để thích nghi sang lời tiếng Việt + phong cách. Tránh hoàn toàn vấn đề
   không tương thích head_dim/RoPE ở hướng 1 (adapter gắn lên chính layer gốc của teacher,
   không cần cắt/chiếu gì). Đánh đổi: mỗi step vẫn phải forward qua nguyên khối 1.14 tỉ tham số
   (không rẻ hơn về compute/step so với distillation hiện tại), nhưng có thể cần **ít step hơn
   nhiều** vì không phải học nhạc/hát từ số 0 — chưa đánh giá feasibility chi tiết, cần làm ở
   phiên sau.
3. **Hai kỹ thuật TTS trực tiếp nhắm vào độ chính xác tiếng Việt**:
   - **Điều kiện F0 gắn với thanh điệu** (tone-aware pitch conditioning) — tiếng Việt có 6
     thanh, mỗi thanh quy định một dạng đường pitch riêng; nói sai thanh = sai nghĩa. XPhoneBERT
     (đã dùng) mã hoá thanh ở tầng phoneme, nhưng nếu không conditioning trực tiếp bằng đường F0
     khớp thanh, thông tin dễ bị loãng qua style conditioning toàn cục hiện tại. Đây nâng ưu
     tiên của việc "khôi phục điều kiện F0" (đã nêu trên) — không chỉ là tín hiệu phụ mà là kỹ
     thuật chuẩn cho đúng vấn đề "khớp tiếng Việt".
   - **Duration/alignment modeling rõ ràng** (kiểu FastSpeech2/VITS) — dự đoán thời lượng riêng
     cho mỗi âm vị, học từ alignment cấp phoneme (không chỉ cấp câu/đoạn như Whisper timestamp
     hiện tại). Khớp trực tiếp với phát hiện ở spectrogram (§4.22): output hiện tại thiếu hẳn
     khoảng lặng/cấu trúc thời gian mà vocal thật có — model chưa từng được dạy "khi nào nên có
     khoảng lặng" ở độ chi tiết phoneme.
4. **Sinh dữ liệu tổng hợp bằng chính teacher (self-distillation với synthetic data)** — dùng
   pipeline gốc của teacher (đã hoạt động, §4.22) để tự sinh nhiều "bài hát" tiếng Việt tổng hợp
   (đổi lời/seed/style), đưa qua đúng pipeline tiền xử lý mel hiện tại, dùng làm dữ liệu train bổ
   sung — nhân rộng hiệu quả 250 bài mà "nhãn" đến từ một model đã chứng minh sinh sạch. Rẻ hơn
   hẳn so với sourcing/preprocess thêm dữ liệu thật (không cần Demucs/Whisper cho audio thật mới)
   nhưng chất lượng phụ thuộc hoàn toàn vào chất lượng sinh của teacher — nên xem là **bổ sung**
   cho việc mở rộng dữ liệu thật (ưu tiên #1 ở trên), không thay thế hoàn toàn.

Cả 4 hướng đều **chưa thực hiện** — chỉ mới đánh giá khả thi/đề xuất trong lúc chờ quota mới.
Không nên coi đây là kế hoạch đã chốt, mà là danh sách ứng viên cụ thể, có cơ sở kỹ thuật, cho
