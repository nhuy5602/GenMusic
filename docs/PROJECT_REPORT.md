...
+ epoch với loss mới + `alpha_feature=0.8` (§4.15-4.16). Phát hiện quan trọng nhất hiện tại:
`voiced_ratio` (chỉ số đã dùng để chọn "cấu hình tối ưu" ở §4.14) có một góc mù — nó không
phân biệt được "hát có giai điệu" với "giữ đứng yên một nốt". Đo thêm `pitch_std_semitones`
(§4.16) cho thấy cấu hình từng được chọn là tối ưu (`dim=256`, 25 epoch) thực ra đơn điệu
nhất trong 4 cấu hình đã thử — khuyến nghị hiện tại là scale `dim=384`/`dim=512` thay vì
`dim=256`, chờ xác nhận bằng nghe thật.
...
hơn việc giữ khả năng so sánh 1:1 với các con số cũ. *(Kết quả cập nhật tiếp khi job hoàn tất.)*
