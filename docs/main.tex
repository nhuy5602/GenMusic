...
mô tả đúng thứ tự các phép biến đổi trong lượt truyền tiến.
...
lyric segment giao với crop, ghép thành một chuỗi ký tự, rồi bộ mã hoá âm vị tiền huấn luyện
tự chạy G2P và tokenize bên trong lượt truyền tiến. Style prompt là vector 512 chiều đã
...
Text sau projection $E_l$ & $B\times L\times D$ & MLP trainable $768\rightarrow D\rightarrow D$ rồi một lớp self-attention trainable riêng cho ca từ; giữ nguyên chiều dài $L$ (không nội suy theo $T$, vì dùng cross-attention). \\
...
Ca từ được chuyển thành âm vị bằng bộ chuyển tự vị--âm vị (G2P tiếng Việt, có dấu
thanh) trước khi tokenize bằng chính tokenizer của XPhoneBERT
\cite{xphonebert}. Bộ mã hoá âm vị tiền huấn luyện sau đó chạy chuỗi token qua
XPhoneBERT — toàn bộ tham số của transformer này bị đóng băng, không tham gia lan
truyền ngược — trả hidden state $H_l\in\mathbb{R}^{B\times L\times768}$.
...
$E_l$ đi qua \textbf{một lớp self-attention trainable riêng cho ca từ}
...
từ đầu trên ngân sách $\sim$250 bài (đúng lỗi mà biến thể thử nghiệm học ca từ bằng
một bảng embedding từ đầu từng mắc phải). Kết quả cuối cùng mới được đưa trực tiếp làm key/value cho lớp cross-attention trong
mỗi transformer block (\S\ref{sec:crossattn_block}) — mel chỉ đóng vai trò query. Hệ thống
cũng tính một vector trung bình trên trục token (sau lớp self-attention) làm
style fallback khi không có style prompt; trong huấn luyện chính, style
...
\end{equation}
Style prompt $s\in\mathbb{R}^{512}$ đi qua một bộ mã hoá phong cách và một MLP thứ
hai:
\begin{equation}
...
khối nhúng đầu vào (không nối/concat). Điều kiện thời gian và phong cách vì vậy tác
động ở hai vị trí: ngay tại đầu vào transformer và tại phép chuẩn hóa cuối.

\subsection{Khối hợp nhất điều kiện}

Khối nhúng đầu vào chiếu mel về $D$ chiều rồi cộng cùng time/style embedding
(đã mở rộng theo $T$):
\begin{equation}
...
Mỗi khối Transformer (các khối xây dựng kiểu Llama: self-attention, MLP,
RMSNorm) gồm ba bước tuần tự, kiểu pre-norm:
\begin{enumerate}[label=(\alph*)]
 ...
 \item \textbf{Cross-attention từ mel tới ca từ}: mel (đã qua self-attention) làm query,
   $E_l$ làm key/value; cơ chế attention chuẩn nhiều đầu (không dùng RoPE, khác với
   self-attention trên mel), padding của ca từ được mask:
   \begin{equation}
    H''=H'+\operatorname{CrossAttn}(\operatorname{RMSNorm}(H'),\,E_l,\,E_l,\,\text{mask}_{\mathrm{text}}).
   \end{equation}
 \item \textbf{Feed-forward} (SiLU, hệ số mở rộng $r$, kích thước trung gian $D\times r$):
   \begin{equation}
...
Sau block cuối, lớp chuẩn hoá thích ứng cuối dùng $c=E_t+E_s$ để sinh scale và shift:
\begin{align}
...
\end{equation}
Đầu phụ trợ chỉ được tính khi được yêu cầu tường minh và chỉ
...
Attention heads & 4 & 4 \\
Hệ số FFN & 4 & 4 \\
Text encoder & XPhoneBERT + G2P, đóng băng & giống mặc định \\
...
encoder nhỏ mới, khoảng 11 triệu tham số.

Encoder gồm một lớp tích chập đầu vào và năm khối giảm mẫu liên tiếp, mỗi khối gồm ba đơn
vị residual với hệ số giãn nở $1$, $3$, $9$ và một lớp tích chập có bước nhảy; tích các
bước nhảy của năm khối bằng $4800$, khớp chính xác tỉ lệ nén encode-side thật của teacher
(xác nhận qua cấu hình công khai của decoder). Số kênh tăng dần từ 32 lên 512 qua các khối,
sau đó một lớp tích chập cuối tách đặc trưng 512 chiều thành $\mu$ và $\log\sigma^2$ (64
chiều mỗi phần, xem \S\ref{sec:vae_bottleneck}). Encoder train trực tiếp trên waveform
24~kHz gốc (vocal+backing đã tách qua Demucs), không qua bước trung gian mel/Vocos nào —
tránh mất pha do một vòng vocoder phụ không cần thiết. Loss reconstruction là trung bình
L1-trên-log-mel qua ba thang phân giải STFT khác nhau, so giữa audio decoder tái tạo và
audio thật — không có số hạng đối kháng (rủi ro bất ổn GAN training, cố ý để ngoài phạm vi
đề tài).
...
Pipeline chưng cất tải cấu hình thật đi kèm checkpoint thay vì giả định số mel bins. Teacher được đóng băng và chuyển sang evaluation mode. Lyric token, tokenizer, thứ tự nối chuỗi, sentinel timestep $-1$ và attention mask đều theo triển khai chính thức. Chế độ huấn luyện chưng cất yêu cầu teacher và tokenizer thật; fallback giả chỉ được phép trong kiểm thử riêng, nhằm ngăn một lần chạy mang nhãn distillation nhưng thực chất không có tín hiệu teacher. Student luôn là MicroDiT (backbone duy nhất trong kiến trúc hiện tại, \S\ref{sec:microdit_architecture}), chạy được trên cả mel-space và latent-space (\S\ref{sec:latent_space}).
...
\end{equation}
Phần truyền tiến của teacher không tham gia lan truyền ngược, nhưng lớp ánh xạ $A_\phi$ nằm
ngoài phạm vi đó để nhận gradient. Đây là chi tiết quan trọng: nếu để lớp ánh xạ này không
nhận gradient, phép ánh xạ ban đầu không bao giờ thích nghi và student phải bắt chước một
mục tiêu tùy ý -- đúng lỗi từng gặp phải ở một phiên bản trước đó của hệ thống, khi hai lớp
ánh xạ mel bị đặt nhầm ngoài phạm vi lan truyền ngược dù đã được đăng ký để huấn luyện.
...
đúng số chiều của teacher — hệ thống tự động bỏ qua bước resize mel lẫn bước resample thời
gian, vì hai phía đã cùng một biểu diễn ngay từ đầu; resample một chuỗi đã ở 5~Hz theo đúng
công thức dành cho mel-space (giả định 93,75~Hz) sẽ làm hỏng nó thêm một lần nữa, không
phải chỉnh đúng. Khoảng cách phân phối $93{,}75/5=18{,}75$ lần giữa hai tần số vì vậy không
còn tồn tại.
...
Cho student không gian latent thật của teacher cải thiện rõ rệt SD cao độ, xác
nhận thêm bằng nghe thử trực tiếp (2026-07-22) -- lần đầu tiên trong đề tài một mẫu CFM
cho giai điệu nghe được thay vì nhiễu/đơn điệu. Tuy nhiên (C) chưa hội tụ và dùng backbone
đã bị loại khỏi kiến trúc cuối cùng, nên số liệu ở bảng \ref{tab:mel_vs_latent} là tạm
thời; kết quả đầy đủ trên MicroDiT sẽ có sau khi huấn luyện chưng cất hoàn tất
...
cho các giá trị $\beta$ nhỏ hơn.

\subsection{Trả lời}

Bottleneck xác suất (VAE) là cần thiết để tránh collapse ở quy mô dữ liệu lớn, nhưng chỉ
khi kết hợp lịch trình cyclical annealing thay vì $\beta$ hằng số, và cần dò
theo từng bước nhỏ vì tồn tại điểm tối ưu không đơn điệu. Encoder
$\beta=0{,}15$ là checkpoint tốt nhất hiện có, dùng làm đầu vào cho bước CFM
training/distillation cuối cùng (Chương~\ref{chapter:conclusion}).
