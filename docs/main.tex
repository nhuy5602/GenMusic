...
trên HuggingFace) nhưng không công bố encoder tương ứng. Thay vì train lại một VAE đầy đủ
đúng-như-paper (có discriminator đối kháng, quá tốn/rủi ro cho ngân sách của đề tài),
phương pháp giữ decoder \textbf{đông lạnh} (đã pretrained, chính là decoder thật của
teacher) và chỉ train một encoder nhỏ mới:
\begin{itemize}
 \item \textbf{\texttt{LatentAudioEncoder}} (11M tham số): \texttt{Conv1d}$(1{\to}32)$, 5
   ...
   với stride $(10,10,8,3,2)$ — tích $=4800$, khớp chính xác tỉ lệ nén encode-side thật của
   teacher (xác nhận qua \texttt{upsample\_rates} của \texttt{decoder.json}) — kênh tăng
   dần $32{\to}64{\to}128{\to}256{\to}512$ (chặn ở 512), rồi \texttt{Conv1d}$(512{\to}128)$
   tách thành $\mu$/$\log\sigma^2$ (64 chiều mỗi phần, xem \S\ref{sec:vae_bottleneck}).
 \item Encoder train trực tiếp trên waveform 24~kHz gốc (vocal+backing đã tách qua
   Demucs), không qua bước trung gian mel/Vocos nào — tránh mất pha do một vòng vocoder
   phụ không cần thiết (\texttt{preprocess-raw --raw-audio}).
 \item Loss reconstruction: trung bình L1-trên-log-mel qua 3 thang STFT
   $(n_{\mathrm{fft}},n_{\mathrm{mel}})\in\{(512,40),(1024,80),(2048,80)\}$, so giữa audio
   decoder tái tạo và audio thật — không có số hạng đối kháng (rủi ro bất ổn GAN training,
   cố ý để ngoài phạm vi đề tài).
\end{itemize}

\subsection{Bottleneck xác suất (VAE) và lịch trình KL}
\label{sec:vae_bottleneck}

Một encoder chỉ train bằng loss reconstruction thuần (không bước xác suất, không
adversarial loss) có xu hướng kinh điển "regression-to-the-mean" khi target đa dạng — cùng
lớp bệnh lý đã gặp ở CFM loss (\S4.11--4.13, \texttt{docs/project\_history.md}, dẫn
Dieleman 2024/DMD-ADM). Để tránh, \texttt{LatentAudioEncoder.forward()} áp dụng
reparameterization trick chuẩn của VAE: $z=\mu+\sigma\epsilon$ lúc train (backprop qua
sample thật), $z=\mu$ lúc eval (hành vi xác định cho suy luận), với hàm mất mát tổng
\begin{equation}
 \mathcal{L} = \mathcal{L}_{\text{recon}} + \texttt{kl\_weight}\cdot\mathcal{L}_{\mathrm{KL}},
 \quad \mathcal{L}_{\mathrm{KL}}=-\tfrac12\,\mathrm{mean}\big(1+\log\sigma^2-\mu^2-\sigma^2\big).
\end{equation}

Huấn luyện với \texttt{kl\_weight} hằng số dẫn tới hiện tượng \textbf{``KL vanishing''}
(Fu et al., \emph{Cyclical Annealing Schedule: A Simple Approach to Mitigating KL
Vanishing}, NAACL 2019): mạng thu nhỏ $\sigma$ về gần 0 để loại bỏ chi phí mà nhiễu
reparameterization gây ra cho reconstruction, khiến bottleneck xác suất trở thành vô tác
dụng dù công thức toán học đúng (kết quả định lượng ở Thí nghiệm 5, \S\ref{sec:exp5_vae}).
Rivera (\emph{How to train your VAE}, arXiv:2309.13160) chỉ ra nguyên nhân sâu hơn: KL
chuẩn gộp chung việc phạt phương sai cá nhân collapse về 0 (cần thiết, tránh encoder tất
định hoá) với việc phạt trung bình cá nhân dạt về 0 (không cần thiết, xoá thông tin
per-sample). Để giảm thiểu KL vanishing mà không cần tách riêng hai số hạng đó (undertaking
lớn hơn), \texttt{kl\_weight} được áp dụng theo lịch trình \textbf{cyclical annealing} (Fu
et al.) thay vì hằng số: tổng số step huấn luyện chia thành 4 chu kỳ, mỗi chu kỳ ramp tuyến
tính $0\to\texttt{kl\_weight}$ ở nửa đầu rồi giữ nguyên ở nửa sau — cho decoder liên tục có
cơ hội phụ thuộc lại vào $z$ giàu thông tin thay vì ổn định vào trạng thái phớt lờ nó vĩnh
viễn.

\subsection{Mở rộng dữ liệu và hạ tầng huấn luyện}
\label{sec:raw_audio_encoder}

\texttt{train-latent-encoder} được mở rộng để nhận nhiều thư mục dataset cùng lúc (giống
\texttt{train-self}), cho phép gộp toàn bộ 6 phần của corpus gốc
(\texttt{sonlest/vietnamese-music-dataset-version3}, 1839 bài sau lọc chất lượng) thành
một lần huấn luyện duy nhất. \texttt{DataLoader} dùng \texttt{num\_workers=4} +
\texttt{pin\_memory} để chồng lấp I/O (file waveform thô nặng hơn mel tương ứng đúng
$256/100=2{,}56$ lần) với tính toán GPU. \texttt{--resume-checkpoint} cho phép tiếp tục
huấn luyện từ một checkpoint có sẵn với \texttt{kl\_weight} khác (tối ưu hoá bậc thang cho
\texttt{kl\_weight}, \S\ref{sec:exp5_vae}), tránh phải trả lại chi phí của các epoch đầu.

Trước khi chạy full, hạ tầng được kiểm chứng trên GPU Kaggle thật: (1) đọc thử tensor từ cả
6 phần dataset (1839 record, không NaN, đúng schema); (2) một lần train nhỏ với 1 bài lấy
từ mỗi phần trong cùng 1 epoch, xác nhận việc gộp nhiều dataset hoạt động đúng trên dữ liệu
thật; (3) smoke-test VAE bottleneck (6 bài, 1 epoch) cho \texttt{recon\_loss}/\texttt{kl\_loss}
hữu hạn, không NaN, VRAM khớp benchmark cũ. Kiểm chứng cục bộ độc lập (29/29 unit test) xác
nhận eval mode xác định, train mode sample ngẫu nhiên đúng, và gradient chảy ngược qua
sample. \textbf{Sanity-check bắt buộc trước khi tin tưởng bất kỳ checkpoint nào}
(\texttt{scripts/check\_latent\_encoder\_quality.py}): giải mã trực tiếp vài latent
ground-truth (bỏ qua CFM student), đo \texttt{pitch\_std\_semitones} so với chính audio
thật của cùng bài — chỉ số duy nhất phân biệt được encoder collapse (gần một cao độ cố
định) với encoder khoẻ mạnh, vì loss reconstruction giảm mượt không đảm bảo điều này (đã
xảy ra thật, xem Thí nghiệm 5). Kết quả định lượng đầy đủ của toàn bộ quá trình lựa chọn
\texttt{kl\_weight} (bốn giá trị, gồm cả hai lần collapse thật và điểm tối ưu tìm được)
được trình bày ở Thí nghiệm 5 (\S\ref{sec:exp5_vae}); checkpoint tốt nhất từ đó là checkpoint
dùng cho bước CFM training tiếp theo (\S\ref{chapter:conclusion}).
...
\begin{description}
 ...
 \item[RQ5:] Bottleneck xác suất (VAE, mu/logvar + KL divergence) có khắc phục được hiện tượng collapse của latent encoder ở quy mô dữ liệu lớn hay không, và trọng số \texttt{kl\_weight} nào là tối ưu?
\end{description}
...
Encoder ở Thí nghiệm 4 dòng (B) (\S\ref{sec:latent_space}) collapse hoàn toàn khi chỉ dùng
loss reconstruction thuần — giải mã latent ground-truth ra gần một cao độ cố định
(\texttt{pitch\_std\_semitones}$\approx0{,}44$--$0{,}78$ qua hai lần thử warmup/learning-rate
khác nhau). Thí nghiệm này kiểm tra giả thuyết đã đưa ra ở \S\ref{sec:vae_bottleneck}: thêm
bottleneck xác suất (mu/logvar + KL divergence) có khắc phục được collapse hay không, và
\texttt{kl\_weight} nào là tối ưu. Bốn cấu hình được so sánh, cùng huấn luyện trên toàn bộ
1839 bài (10 epoch cho hai cấu hình đầu; 5 epoch \texttt{--resume-checkpoint} tiếp theo cho
hai cấu hình sau, xem \S\ref{sec:raw_audio_encoder}), đo bằng ba chỉ số trên audio thật (không
qua CFM student, cô lập riêng chất lượng encoder): $\sigma_{\text{mean}}=\exp(0{,}5\log\sigma^2)$
(bottleneck có thật sự xác suất không), \texttt{pitch\_std\_semitones} giải mã (chất lượng
reconstruction), và khoảng cách $\mu$ giữa các bài khác nhau $\lVert\mu_A-\mu_B\rVert$ per-element
(encoder có phân biệt được các bài hay bị "mean collapse" — kiểm tra một giả thuyết bên ngoài,
xem diễn giải).
...
\begin{table}[H]
\centering
\caption{Ablation \texttt{kl\_weight} cho bottleneck xác suất của \texttt{LatentAudioEncoder}}
\label{tab:kl_ablation}
\small
\begin{tabularx}{\textwidth}{Xrrrl}
\toprule
\textbf{Cấu hình} & $\boldsymbol{\sigma_{\text{mean}}}$ & \textbf{pitch\_std} & $\boldsymbol{\mu}$\textbf{-distance} & \textbf{Ghi chú} \\
\midrule
Không VAE (2 lần thử) & — & 0,44 / 0,78 & — & collapse hoàn toàn \\
\texttt{kl\_weight}$=10^{-4}$ hằng số & 0,003--0,005 & 5,85$^{*}$ & — & $^{*}$KL vanishing: $\sigma\approx0$, bottleneck vô tác dụng dù pitch\_std trông tốt \\
\texttt{kl\_weight}$=0{,}05$ cyclical & 0,11--0,14 & 4,72 & 0,80--0,91 & VAE thật hoạt động lần đầu \\
\texttt{kl\_weight}$=0{,}15$ cyclical & 0,245--0,273 & \textbf{6,01} & 0,84--0,91 & \textbf{tốt nhất} \\
\texttt{kl\_weight}$=0{,}3$ cyclical & 0,34--0,37 & 3,72 & 0,81--0,88 & vượt ngưỡng, over-regularization \\
\bottomrule
\end{tabularx}
\end{table}
\emph{Mốc tham chiếu: audio thật cùng 5 bài cho pitch\_std\_semitones trung bình $9{,}46$.
...
\textbf{KL vanishing là một false positive nguy hiểm.} Ở \texttt{kl\_weight}$=10^{-4}$ hằng
số — giá trị mặc định ban đầu, và cũng là giá trị Stable Audio 2.0 dùng thật — encoder cho
\texttt{pitch\_std}$=5{,}85$, trông như một thành công rõ rệt so với hai lần collapse trước.
Nhưng đọc trực tiếp $\sigma$ cho thấy bottleneck xác suất đã collapse về gần 0
($\sigma_{\text{mean}}\approx0{,}003$--$0{,}005$): mạng đã học cách loại bỏ nhiễu
reparameterization để tối ưu reconstruction thuần tuý, khiến \texttt{pitch\_std} tốt \emph{vì}
encoder gần như tất định — hiện tượng \textbf{"KL vanishing"} (Fu et al., NAACL 2019). Đây là
bằng chứng thực nghiệm cho lý do vì sao Stable Audio 2.0 cần thêm adversarial loss (5
discriminator) và huấn luyện encoder+decoder cùng nhau để dùng an toàn cùng \texttt{kl\_weight}
này — hai điều kiện project này cố ý không có (\S\ref{sec:latent_encoder_arch}), nên khoảng
\texttt{kl\_weight} an toàn dịch chuyển lên cao hơn nhiều.

\textbf{Lịch trình cyclical annealing khắc phục được KL vanishing, và tồn tại một điểm tối ưu
không đơn điệu.} Từ \texttt{kl\_weight}$=0{,}05\to0{,}15$, cả $\sigma$ và \texttt{pitch\_std}
cùng tăng — bottleneck xác suất càng hoạt động thật, reconstruction càng tốt hơn (không đánh
đổi). Nhưng từ $0{,}15\to0{,}3$, $\sigma$ tiếp tục tăng trong khi \texttt{pitch\_std}
\textbf{giảm} — đúng hiện tượng over-regularization kinh điển của VAE (Rivera, 2023: trọng số
KL quá lớn "over-regularize the latents, degrading reconstruction quality"). \textbf{Kết luận:
$\texttt{kl\_weight}=0{,}15$ là điểm tối ưu trong các giá trị đã thử}, không phải giá trị lớn
nhất hay nhỏ nhất — checkpoint này được dùng cho các bước huấn luyện tiếp theo.

\textbf{Kiểm định giả thuyết "mean collapse" từ một nhận xét bên ngoài.} Một nhận xét cho rằng
vấn đề là $\mu$ của mọi bài hội tụ về gần điểm $0$ do \texttt{kl\_weight} quá \emph{lớn}, đề
xuất giảm xuống $10^{-5}$--$10^{-6}$ — ngược hướng với kết quả bảng \ref{tab:kl_ablation}. Đo
trực tiếp $\lVert\mu_A-\mu_B\rVert$ giữa 5 bài khác nhau (cột thứ tư) cho khoảng cách
$0{,}80$--$0{,}91$, xấp xỉ bằng chính độ lệch chuẩn nội tại của một bài ($\approx1{,}0$) — theo
đúng ngưỡng đề xuất trong nhận xét đó (collapse nếu $<0{,}5$), số liệu không ủng hộ giả thuyết
mean collapse. Bài học phương pháp luận: đối chiếu mọi giả thuyết/gợi ý — kể cả nghe hợp lý về
mặt lý thuyết — với số liệu đo trực tiếp trên checkpoint thật trước khi hành động theo nó.

\subsection{Trả lời}

Bottleneck xác suất (VAE) là cần thiết để tránh collapse ở quy mô dữ liệu lớn, nhưng bản thân
việc thêm nó không đủ nếu \texttt{kl\_weight} là hằng số — cần lịch trình cyclical annealing để
tránh KL vanishing, và cần dò \texttt{kl\_weight} theo từng bước nhỏ (không đơn điệu: có một
điểm tối ưu, tăng thêm sau điểm đó gây hại). Encoder \texttt{kl\_weight}$=0{,}15$ là checkpoint
tốt nhất hiện có của đề tài, dùng làm đầu vào cho bước CFM training/distillation cuối cùng
...
Đối với RQ5 (\S\ref{sec:exp5_vae}), bottleneck xác suất khắc phục được collapse của encoder,
nhưng chỉ khi kết hợp đúng lịch trình cyclical annealing thay vì trọng số KL hằng số — thử với
trọng số hằng số mặc định ($10^{-4}$) cho một kết quả trông tốt (\texttt{pitch\_std}$=5{,}85$)
nhưng thực chất là "KL vanishing" (bottleneck xác suất collapse về gần tất định), một false
positive chỉ phát hiện được bằng cách đọc trực tiếp $\sigma$ chứ không phải qua chỉ số âm thanh
gián tiếp. Sau khi sửa, tồn tại một điểm tối ưu không đơn điệu cho \texttt{kl\_weight}
($0{,}15$ trong các giá trị đã thử) — kết quả này củng cố thêm bài học đã rút ra ở RQ3/RQ4:
mọi chỉ số khách quan, kể cả khi trông "tốt", cần được đối chiếu với phép đo trực tiếp vào
tham số nội bộ của mô hình trước khi tin tưởng.
...
biểu diễn phổ/latent, validation, huấn luyện, distillation, sampling và giải mã
waveform. Ở giai đoạn đầu của đề tài, việc đồng nhất cấu hình mel-spectrogram với
đầu vào gốc của Vocos (Thí nghiệm 1, \S\ref{sec:e1_vocoder}) là bước kiểm chứng nền
tảng giúp loại bỏ triệt để hiện tượng méo âm khi giải mã, qua đó xác nhận tính đúng
đắn của hợp đồng biểu diễn âm thanh trước khi mở rộng sang các thí nghiệm phức tạp
hơn -- đây là một bước sanity-check cần thiết, không phải kết quả cuối cùng của đề
tài. Kết quả có tác động rõ nhất tới chất lượng sinh nhạc thực tế đến từ hướng đi
sau đó: chuyển sang không gian latent thật của teacher và khắc phục collapse của
bộ mã hóa bằng bottleneck xác suất (Thí nghiệm 4--5, \S\ref{sec:exp4_mel_vs_latent},
\S\ref{sec:exp5_vae}).
...
hạ tầng đọc/gộp nhiều dataset trên GPU Kaggle thật. Hai lần huấn luyện lại đầu tiên
trên toàn bộ dữ liệu này — chỉ dùng loss reconstruction thuần, không có bước xác
suất — đều \textbf{collapse} khi sanity-check bằng cách giải mã trực tiếp latent
ground-truth (\texttt{pitch\_std\_semitones} chỉ $0{,}44$ rồi $0{,}78$ trung bình
qua hai lần thử warmup/learning-rate khác nhau, so với $9{,}46$ của audio thật).
Đây là bằng chứng thứ hai, độc lập với lần collapse đầu (\S\ref{sec:latent_space}),
cho cùng một bài học: loss reconstruction thấp/mượt không đủ để tin một encoder
khoẻ mạnh — sanity-check bằng ground-truth-decode là bước bắt buộc, không phải thủ
tục hình thức. Hai lần thất bại liên tiếp với hyperparameter khác nhau cho thấy đây
không đơn thuần là thiếu warmup/LR quá cao, mà là thiếu hẳn một cơ chế cản mạng tự
thu hẹp về nghiệm tất định khi dữ liệu đa dạng hơn.

Nguyên nhân sâu hơn được xác định và khắc phục trong Thí nghiệm 5
(\S\ref{sec:exp5_vae}): thêm bottleneck xác suất thật (mu/logvar + KL divergence,
\S\ref{sec:vae_bottleneck}) cho \texttt{LatentAudioEncoder}, cùng lịch trình
\emph{cyclical KL annealing} để tránh hiện tượng "KL vanishing" mà một trọng số KL
hằng số gây ra. Ablation trên bốn giá trị \texttt{kl\_weight} xác nhận
$\texttt{kl\_weight}=0{,}15$ khắc phục được collapse ngay ở quy mô đầy đủ 1839 bài
(\texttt{pitch\_std\_semitones}$=6{,}01$, so với $0{,}44$--$0{,}78$ khi không có
bottleneck xác suất) và là điểm tối ưu trong các giá trị đã thử. Checkpoint encoder
này -- không phải checkpoint 249 bài trước đó -- được dùng làm đầu vào cho lượt
huấn luyện chưng cất (\texttt{train-distill}) quy mô đầy đủ tiếp theo, sử dụng
teacher DiffRhythm2 thật trên toàn bộ dữ liệu đã precompute latent. Tại thời điểm
hoàn thành báo cáo, quá trình huấn luyện chưng cất này đang chạy nhưng chưa đủ số
liệu hội tụ để đánh giá chất lượng cuối cùng; đây là hướng cần được resume và đánh
giá đầy đủ (WER, \texttt{pitch\_std}) ngay khi có thêm ngân sách GPU
...
\begin{enumerate}[label=\arabic*)]
    \item Việc mở rộng dữ liệu encoder từ 249 lên 1843 bài (\S\ref{sec:raw_audio_encoder}) ban
      đầu gây collapse hai lần liên tiếp, nhưng đã được khắc phục bằng bottleneck xác suất (VAE)
      và lịch trình cyclical KL annealing (Thí nghiệm 5, \S\ref{sec:exp5_vae}), với
      $\texttt{kl\_weight}=0{,}15$ là cấu hình tốt nhất tìm được. Hướng phát triển tiếp theo
      không còn là tìm công thức chống collapse, mà là: (a) huấn luyện chưng cất
      (\texttt{train-distill}) trên checkpoint encoder này tới hội tụ thật, thay vì bị giới hạn
      bởi thời gian/quota GPU như lần chạy hiện tại; (b) đánh giá bằng cả \texttt{pitch\_std} lẫn
      chỉ số độ rõ lời (PhoWhisper WER) trên nhiều mẫu hơn, không chỉ một vài bài; và (c) nếu còn
      dư địa, thử tách riêng số hạng KL\_G/KL\_I theo Rivera (arXiv:2309.13160) thay vì KL chuẩn,
      để giảm rủi ro over-regularization đã quan sát được ở $\texttt{kl\_weight}=0{,}3$.
