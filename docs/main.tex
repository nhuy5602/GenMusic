...
tối ưu hoá}. Thêm KL/adversarial loss là hướng khắc phục thật nhưng là một undertaking lớn, đúng
rủi ro đã lường trước từ đầu (\S\ref{sec:latent_space}) — ngoài phạm vi thời gian của report này.

\textbf{Quyết định}: dừng thử thêm hyperparameter ở quy mô 1839 bài, quay lại dùng checkpoint
encoder 249 bài đã xác nhận khoẻ mạnh (\S\ref{sec:latent_space}) để tiến hành bước còn treo lâu
nay — huấn luyện \texttt{MicroDiT} bằng CFM trực tiếp trên latent-space (chưa từng chạy, mọi kết
quả latent-space trước đó đều từ \texttt{NativeDiTStudent} đã gộp/xoá,
\S\ref{sec:native_dit}) — xem kết quả ở \S\ref{chapter:conclusion}. Việc mở rộng dữ liệu encoder
lên quy mô lớn ổn định là một hướng phát triển riêng, chưa khép lại trong report này.
