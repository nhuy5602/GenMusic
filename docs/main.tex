...
hạ tầng đọc/gộp nhiều dataset trên GPU Kaggle thật. Encoder đang được huấn luyện lại
trên toàn bộ dữ liệu này tại thời điểm viết báo cáo; đây là bước điều kiện tiên
quyết trước khi resume CFM training tới hội tụ như đã đề ra ở lần viết report trước.
...
\begin{enumerate}[label=\arabic*)]
    \item Đã hoàn thành: mở rộng dữ liệu encoder từ 249 lên 1843 bài và loại bỏ vòng
      giải mã Vocos trung gian (\S\ref{sec:raw_audio_encoder}). Đang thực hiện: huấn
      luyện lại \texttt{LatentAudioEncoder} trên toàn bộ 1843 bài; sau khi sanity-check
      (giải mã latent ground-truth, đúng quy trình \S4.24) xác nhận encoder ổn định,
      huấn luyện tiếp \texttt{MicroDiT} bằng CFM trên không gian latent này tới khi
      early stopping tự kích hoạt thật, thay vì bị cắt ngang do hết quota Kaggle như
      lần chạy trước (checkpoint \texttt{NativeDiTStudent}, \S\ref{sec:exp4_mel_vs_latent}),
      rồi điền lại số liệu ở bảng \ref{tab:mel_vs_latent}.
