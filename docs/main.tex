...
Sau khi xác nhận đúng đắn, mới chạy full 1839 bài/10 epoch — kết quả ở \S\ref{chapter:conclusion}.

Song song, một checkpoint tạm dùng encoder 249 bài đã xác nhận khoẻ mạnh
(\S\ref{sec:latent_space}) đã được bắt đầu để không bỏ trống bước \texttt{MicroDiT}+CFM trên
latent-space (chưa từng chạy, mọi kết quả latent-space trước đó đều từ \texttt{NativeDiTStudent}
đã gộp/xoá, \S\ref{sec:native_dit}) — nhưng bị dừng lại để ưu tiên xác nhận kết quả của encoder
VAE mới trước, theo đúng nguyên tắc kiểm chứng từng bước trước khi đi tiếp. Xem kết quả cuối cùng
(encoder VAE có ổn định không, và bước CFM/distillation tiếp theo) ở \S\ref{chapter:conclusion}.
