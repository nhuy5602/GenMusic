...
MicroDiT (\texttt{src/models/dit\_transformer.py}) là backbone mặc định học trường vận
tốc trong hệ thống (kiến trúc thay thế \texttt{NativeDiTStudent} được trình bày ở
\S\ref{sec:native_dit}). Mô hình nhận noisy mel tại thời điểm $t$ cùng hai nhóm điều kiện:
...
Text sau projection $E_l$ & $B\times L\times D$ & MLP trainable $768\rightarrow D\rightarrow D$; giữ nguyên chiều dài $L$ (không nội suy theo $T$, vì dùng cross-attention). \\
...
$E_l$ được \textbf{giữ nguyên chiều dài $L$} và đưa trực tiếp làm key/value cho lớp
cross-attention trong mỗi transformer block (\S\ref{sec:crossattn_block}) — mel chỉ đóng
vai trò query. Mã nguồn cũng tính \texttt{pooled\_text} (trung bình trên trục token) làm
...
Suy luận bắt đầu bằng Gaussian noise $x_0$ có hình dạng $1\times100\times T$. Tại mỗi $t_k$, model (MicroDiT hoặc \texttt{NativeDiTStudent}, \S\ref{sec:native_dit}) dự đoán velocity và Euler solver cập nhật trạng thái (mặc định 32 bước, có thể đổi qua \texttt{--steps}). Sau bước cuối, mel được giải mã trực tiếp bằng Vocos (hoặc bằng decoder BigVGAN thật nếu đang ở không gian latent, \S\ref{sec:latent_space}). Nếu không tải được Vocos, hệ thống chuyển sang Griffin--Lim 64 vòng lặp và ghi rõ backend đã dùng.
...
\section{Kiến trúc thay thế: \texttt{NativeDiTStudent}}
\label{sec:native_dit}

Song song với MicroDiT, dự án hiện thực thêm một backbone thứ hai
(\texttt{src/models/diffrhythm2\_native.py}, chọn qua \texttt{--architecture native\_dit}),
port trực tiếp từ mã nguồn thật của DiffRhythm2 (có ghi nguồn, giấy phép Apache~2.0). Khác
biệt cơ chế chính so với MicroDiT: ca từ và mel \textbf{chia sẻ một chuỗi self-attention
duy nhất} thay vì cross-attention riêng.

Cụ thể: chuỗi âm vị (cùng bước G2P như MicroDiT) được nhúng bằng một \texttt{nn.Embedding}
huấn luyện từ đầu (không dùng transformer XPhoneBERT pretrained) thành $E_l\in\mathbb{R}^{B\times L\times512}$;
mel được chiếu tuyến tính thành $E_a\in\mathbb{R}^{B\times T\times512}$; hai chuỗi được nối
theo trục thời gian $\mathrm{combined}=[E_l;E_a]\in\mathbb{R}^{B\times(L+T)\times512}$. Vị trí
ca từ nhận giá trị timestep \emph{sentinel} $-1$, còn vị trí mel nhận timestep $t$ thật; vị
trí RoPE của hai đoạn đều tự đánh số lại từ $0$ (không tiếp tục liên tục), khớp đúng hành vi
"block đầu tiên" của teacher thật. Attention mask hai chiều đầy đủ trên toàn chuỗi
$[l;a]$, chỉ che padding của ca từ — không có chia block clean/noisy như đường chưng cất
(\S\ref{sec:distill_teacher_call}). Sau \texttt{depth} lớp \texttt{LlamaNARDecoderLayer}
(Llama block chuẩn, có thêm RMSNorm học được trên Q/K) và \texttt{AdaLayerNormZero\_Final},
chỉ phần đầu ra ứng với đoạn mel được giữ lại; đoạn ca từ bị loại sau khi đã đóng góp ngữ
cảnh qua self-attention.

Cùng CLI, cùng \texttt{cfm\_loss}, cùng vòng train/validation/early-stopping với MicroDiT
— chỉ khác lớp model được khởi tạo. Đo thực nghiệm ở cùng kích thước/epoch
(\texttt{docs/project\_history.md} \S4.19): \texttt{loss\_gt} không đổi đáng kể so với
MicroDiT, nhưng chậm hơn $\approx4{,}4$ lần mỗi step (chuỗi self-attention dài hơn, cộng
chi phí G2P chạy trên CPU mỗi batch) — một đánh đổi thật, không phải một chiến thắng rõ
ràng, độc lập với câu hỏi không gian latent ở mục sau.
...
Pipeline chưng cất tải cấu hình thật đi kèm checkpoint thay vì giả định số mel bins. Teacher được đóng băng và chuyển sang evaluation mode. Lyric token, tokenizer, thứ tự nối chuỗi, sentinel timestep $-1$ và attention mask đều theo triển khai chính thức. Chế độ huấn luyện chưng cất yêu cầu teacher và tokenizer thật; fallback giả chỉ được phép trong kiểm thử riêng, nhằm ngăn một lần chạy mang nhãn distillation nhưng thực chất không có tín hiệu teacher. \textbf{Chỉ hỗ trợ MicroDiT} làm student — \texttt{train-distill} chưa có tuỳ chọn \texttt{native\_dit}.
...
\begin{enumerate}[label=(\Alph*)]
 \item \textbf{Mel-space} (\texttt{--architecture microdit}, mốc \emph{exp06} — cấu hình
 ...
 \item \textbf{Latent-space, encoder đã ổn định hoá + CFM student} — checkpoint tốt nhất
   ...
   stopping chưa kích hoạt thật — xem \S\ref{sec:early_stopping_full}).
\end{enumerate}
