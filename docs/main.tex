...
định. Đang thử lại với warmup dài hơn theo tỉ lệ (400 step, $\approx17\%$ trên 5 epoch/2300 step)
và learning rate thấp hơn ($5\times10^{-5}$) tại thời điểm viết báo cáo — xem \S5 để biết kết quả
cuối khi có, hoặc kết luận nếu chưa kịp trước hạn nộp.
...
Nguyên nhân khả dĩ là công thức warmup/learning-rate đã fix được tinh chỉnh cho quy mô dữ
liệu nhỏ hơn (249 bài) không chuyển giao tốt sang quy mô lớn hơn (1839 bài, đa dạng hơn,
không qua lớp làm mượt của Vocos). Một lần thử lại với warmup dài hơn theo tỉ lệ và
learning rate thấp hơn đang được thực hiện; đây là bước điều kiện tiên quyết trước khi
resume CFM training tới hội tụ như đã đề ra ở lần viết report trước — nếu không kịp có
kết quả tốt trước hạn nộp, hướng khuyến nghị là quay lại dùng checkpoint encoder đã xác
nhận khoẻ mạnh trên 249 bài (\S\ref{sec:latent_space}) làm cơ sở cho các bước tiếp theo,
thay vì trì hoãn toàn bộ tiến độ để chờ encoder quy mô lớn hội tụ.

\section{Hướng phát triển}

Dựa trên tiến độ và kết quả, đề tài ưu tiên các hướng phát triển như sau:
\begin{enumerate}[label=\arabic*)]
    \item Đã hoàn thành: mở rộng dữ liệu encoder từ 249 lên 1843 bài và loại bỏ vòng
      giải mã Vocos trung gian (\S\ref{sec:raw_audio_encoder}). Lần huấn luyện lại đầu
      tiên trên quy mô này \textbf{collapse} khi sanity-check (\texttt{pitch\_std\_semitones}
      $0{,}44$ so với $9{,}46$ của audio thật) dù loss trông tốt — đang thử lại với warmup
      dài hơn theo tỉ lệ và learning rate thấp hơn. Nếu lần thử lại (hoặc các lần tiếp theo,
      trong giới hạn thời gian còn lại) xác nhận encoder ổn định, huấn luyện tiếp
      \texttt{MicroDiT} bằng CFM trên không gian latent này tới khi early stopping tự kích
      hoạt thật, thay vì bị cắt ngang do hết quota Kaggle như lần chạy trước (checkpoint
      \texttt{NativeDiTStudent}, \S\ref{sec:exp4_mel_vs_latent}), rồi điền lại số liệu ở
      bảng \ref{tab:mel_vs_latent}. Nếu không kịp trước hạn nộp, hướng khả thi là quay lại
      checkpoint encoder 249-bài đã xác nhận khoẻ (\S\ref{sec:latent_space}) cho các bước
      tiếp theo, và coi việc mở rộng dữ liệu + tìm công thức warmup/LR ổn định ở quy mô lớn
      là một hướng phát triển riêng, chưa khép lại trong report này.
