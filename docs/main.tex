...
\section{Huấn luyện CFM và chưng cất tri thức từ DiffRhythm2}
...
\end{equation}
một lần duy nhất ở giai đoạn tiền tính (\texttt{precompute\_latent\_dataset.py}), \textbf{trước khi} đưa qua bộ mã hoá latent (VAE, \S\ref{sec:latent_encoder_arch}) đã đóng băng decoder. Bộ mã hoá nén $a^{\mathrm{mix}}$ thành $x_1=\mathrm{Enc}(a^{\mathrm{mix}})\in\mathbb{R}^{F\times T}$ với $F=64$ (latent thật của teacher, 5~Hz, \S\ref{sec:latent_space}) -- mỗi bản ghi huấn luyện chỉ còn một latent cả-bài duy nhất, không còn tách vocal/backing riêng ở bước huấn luyện CFM. Hệ thống cần học trường velocity:
\begin{equation}
...
\textbf{Chú ý quan trọng về vai trò của backing}: $a^{\mathrm{acc}}$ (backing) chỉ được dùng để \emph{xây dựng mục tiêu huấn luyện} $x_1$ -- đã hoà vào latent cả-bài từ trước khi huấn luyện (công thức \eqref{eq:reconstruct_full_mix}, \S\ref{sec:latent_space}) — nó **không** là một tensor điều kiện đưa vào mạng $v_\theta$ (xem \S\ref{sec:microdit_architecture} tiếp theo). Do đó model phải học sinh ra cả bài (giọng hát và nhạc đệm) từ trạng thái không điều kiện theo backing, chỉ dựa vào ca từ và style embedding toàn cục. Để tránh model "lãng quên" tín hiệu giọng hát (yếu hơn, thưa hơn nhạc đệm) khi tối ưu trực tiếp trên hỗn hợp, một đầu ra phụ trợ dự đoán riêng velocity của vocal (\emph{vocal auxiliary head}) được huấn luyện đồng thời — chi tiết ở \S\ref{sec:microdit_architecture}. Đầu phụ trợ này cần một mục tiêu vocal-only riêng biệt với $x_1$; dữ liệu tiền tính hiện chỉ giữ lại latent cả-bài (không có latent vocal-only riêng, \S\ref{sec:latent_space}), nên đầu phụ trợ hiện chưa nhận được tín hiệu vocal-only thật sự khác với mục tiêu chính -- một giới hạn hiện tại của cài đặt, không phải của thiết kế kiến trúc.

\textbf{Tổng quan hệ thống.} Hình~\ref{fig:pipeline} mô tả kiến trúc tổng thể của hệ thống. Kiến trúc không phải một chuỗi xử lý tuyến tính đơn giản mà gồm
...
cộng lại (công thức \eqref{eq:reconstruct_full_mix}) rồi mã hoá qua bộ mã hoá latent (VAE,
\S\ref{sec:latent_encoder_arch}) thành $x_1$ (latent cả bài, 64 chiều/5~Hz) mà model học
sinh ra. Whisper tạo nội dung lời cùng các đoạn thời gian, trong khi MuQ-MuLan tạo một
vector phong cách toàn cục. Các đầu ra này được quy về cùng đơn vị thời gian và lưu trong
một record thống nhất. Một mẫu huấn luyện vì vậy gồm bộ $(x_1, l, s)$ — nhưng chỉ $l$ (lời)
và $s$ (style) được đưa vào mạng $v_\theta$ làm điều kiện; backing chỉ tham gia lúc xây
dựng mục tiêu, không phải một tensor điều kiện của model.

Trong lớp học trường vận tốc, sau validation, MicroDiT được chưng cất tri thức: teacher
DiffRhythm2 được gọi tại cùng trạng thái nhiễu và timestep -- teacher và student dùng chung
không gian latent 64 chiều/5~Hz nên velocity của teacher được dùng thẳng làm mục tiêu bổ
sung, không cần lớp ánh xạ số chiều.

Ở lớp suy luận và giải mã, hệ thống khởi tạo Gaussian noise trong không gian latent, gọi
MicroDiT nhiều lần để tích phân ODE bằng Euler, rồi đưa latent cuối vào decoder BigVGAN đã
đóng băng để giải mã ra waveform. Kaggle chỉ là backend
...
Bốn nguyên tắc thiết kế xuyên suốt cách phân lớp này. Thứ nhất, biểu diễn trung gian phải
nhất quán từ tiền xử lý tới decoder, vì sai hợp đồng biểu diễn có thể che lấp hoàn toàn chất
lượng mô hình sinh. Thứ hai, các điều kiện được phân vai: lyrics cho nội dung (qua
cross-attention) và MuQ-MuLan cho phong cách toàn cục; backing chỉ tham gia lúc xây mục
tiêu huấn luyện (hỗn hợp cả bài), không phải một tensor điều kiện của model. Thứ ba, chưng
cất phải được kiểm chứng ở mức đường truyền gradient và hợp đồng teacher trước khi bàn tới
lợi ích chất lượng. Thứ tư, khi cho student một không gian biểu diễn mới (latent thật của
teacher, \S\ref{sec:latent_space}), phải tự kiểm chứng không gian đó bằng cách giải mã
trực tiếp latent ground-truth trước khi tin bất kỳ kết quả CFM training nào trên nó — một
encoder suy biến có thể trông ổn về mặt kỹ thuật (không NaN, loss giảm) mà vẫn không mang
thông tin âm nhạc nào.

\textbf{Kiến trúc mô hình con.} MicroDiT là backbone \textbf{duy nhất} học trường
vận tốc trong hệ thống. Mô hình nhận
noisy latent (64 chiều/5~Hz, \S\ref{sec:latent_space}) tại thời điểm $t$ cùng
...
-- nối text và latent vào cùng một chuỗi self-attention). Timestep và
style embedding (từ MuQ-MuLan) được kết hợp thành một vector điều kiện chung, vừa cộng
trực tiếp vào latent ở đầu vào vừa điều khiển một phép chuẩn hoá thích ứng (adaptive
...
\textbf{Quá trình huấn luyện và chưng cất.}
...
bằng tín hiệu từ teacher DiffRhythm2 đã đóng băng.
\label{sec:distill_teacher_call}
Pipeline gọi teacher đúng theo cấu hình
...
\textbf{Suy luận và tổng hợp âm thanh.} Suy luận bắt đầu bằng Gaussian noise $x_0$ có hình dạng $1\times64\times T$ (\S\ref{sec:latent_space}). Tại mỗi $t_k$, MicroDiT dự đoán velocity và Euler solver cập nhật trạng thái (mặc định 32 bước, có thể điều chỉnh). Sau bước cuối, latent được giải mã trực tiếp bằng decoder BigVGAN thật đã đóng băng thành waveform 48~kHz.
...
của một Music VAE riêng, ở tần số \textbf{5~Hz}. Hệ thống cho \textbf{student chính nó}
cùng không gian latent nén thật này (\S\ref{sec:microdit_architecture}), độc lập với việc
có chưng cất hay không -- nên teacher và student luôn cùng một biểu diễn, không cần lớp
ánh xạ số chiều hay resample tần số nào giữa hai phía.
...
$1{,}25$ ở epoch 1 xuống còn $\mathbf{0{,}46}$ ở epoch cuối -- không dao động thất thường
như ở Thí nghiệm 3 (crop/timestep cố định mỗi step tại đây thay vì lấy mẫu ngẫu nhiên một
lần mỗi epoch), cho thấy phép hồi quy velocity hội tụ thật chứ không phải một điểm cuối
