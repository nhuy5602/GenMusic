...
Về mặt hạ tầng, toàn bộ pipeline từ tiền xử lý, huấn luyện đến suy luận đã được kiểm chứng vận hành đồng bộ trên dữ liệu tiếng Việt thật. Về mặt biểu diễn trung gian, đề tài xác nhận rằng đưa mô hình sinh về đúng không gian latent nén (64 chiều, 5~Hz) mà DiffRhythm2 thực sự huấn luyện trên -- thay vì mel-spectrogram trung gian ban đầu -- là hướng đi cho tín hiệu tích cực rõ rệt nhất, sau khi giải quyết được hiện tượng collapse của bộ mã hóa latent tự huấn luyện bằng một bottleneck xác suất (VAE) với lịch trình huấn luyện KL phù hợp. Trên nền tảng encoder đó, quá trình chưng cất tri thức từ teacher cho MicroDiT trên không gian latent đã huấn luyện tới hết lịch trình dự kiến (15 epoch), với ground-truth loss giảm liên tục và ổn định từ $1{,}25$ xuống còn $0{,}46$ -- một tín hiệu hội tụ rõ ràng ở mức hàm mất mát. Tuy nhiên, do giới hạn về thời gian, đánh giá chất lượng sinh khách quan (độ rõ lời, độ biến thiên cao độ) trên chính checkpoint hội tụ này chưa kịp hoàn thành tại thời điểm hoàn thành báo cáo. Kết quả hiện tại cho thấy đây là một hướng tiếp cận khả thi và có tiềm năng, với bằng chứng hội tụ ở mức huấn luyện đã rõ ràng; bước tiếp theo cần thiết là xác nhận mức hội tụ đó bằng chỉ số chất lượng sinh khách quan trên nhiều mẫu, thay vì chỉ dựa vào giá trị loss.
...
\begin{tikzpicture}[
    ...
    source/.style={draw, rounded corners=2pt, fill=gray!10,
        minimum height=0.8cm, text width=3.4cm, align=center, font=\scriptsize},
    ...
    artifact/.style={draw, rounded corners=2pt, fill=green!8,
        minimum height=0.9cm, text width=8.8cm, align=center, font=\scriptsize},
    flow/.style={-{Latex[length=2mm]}, line width=0.55pt}
]
\node[source] (audio) at (0,0) {Bài hát thô (WAV/MP3, 24~kHz)};
\node[process] (demucs) at (0,-1.5) {HTDemucs: tách vocal và backing};
\node[process] (whisper) at (-4.2,-3.0) {Whisper: transcript + mốc thời gian (trên vocal)};
\node[process] (mel) at (0,-3.0) {Vocos feature extractor: mel 100 bins (vocal và backing)};
\node[process] (muq) at (4.2,-3.0) {MuQ-MuLan: style embedding 512 chiều (trên bản phối gốc)};
\node[artifact] (record) at (0,-4.7) {Bản ghi huấn luyện đồng bộ $(x_1,b,l,m,s)$: mel vocal, mel backing, ca từ đã căn cửa sổ, mặt nạ token, style embedding};

\draw[flow] (audio) -- (demucs);
\draw[flow] (audio) -| (muq);
\draw[flow] (demucs) -| (whisper);
\draw[flow] (demucs) -- (mel);
\draw[flow] (whisper) |- (record);
\draw[flow] (mel) -- (record);
\draw[flow] (muq) |- (record);
\end{tikzpicture}
...
nhất quán từ tiền xử lý tới vocoder, vì sai hợp đồng mel có thể che lấp hoàn toàn chất
...
trò sinh mục tiêu, không nhận gradient. Khi student chạy ở mel-space, một lệch biểu diễn
nghiêm trọng xuất hiện: teacher chạy ở 5~Hz/64 chiều nhưng student ở 93,75~Hz/100 chiều —
đưa nguyên chuỗi mel-space vào teacher khiến attention của nó chạy ngoài phân phối huấn
luyện (lệch tần số $\approx19$ lần). Một cầu nối giải quyết lệch này bằng cách resample
trục thời gian theo cả hai chiều quanh lượt gọi teacher, khớp lại đúng mặt nạ attention tự
hồi quy theo khối mà teacher được huấn luyện với, và một lớp tuyến tính có thể học ánh xạ
giữa 64 và 100 chiều để bù lệch số kênh, được huấn luyện đồng thời với phần còn lại của
student. Toàn bộ cầu nối này được bỏ qua khi student chạy trên không gian latent thật
64-chiều/5~Hz của teacher (\S\ref{sec:latent_space}), vì khi đó hai phía đã cùng một biểu
diễn ngay từ đầu.
...
kiểm chứng; trên nền không gian latent, chưng cất cho MicroDiT thật đã huấn luyện tới hết
lịch trình dự kiến với ground-truth loss giảm liên tục từ $1{,}25$ xuống $0{,}46$
(\S\ref{sec:exp4_mel_vs_latent}) -- một tín hiệu hội tụ rõ ràng ở mức hàm mất mát, nhưng
bằng chứng về mức cải thiện chất lượng sinh cụ thể (độ rõ lời, độ biến thiên cao độ) trên
đúng checkpoint đó vẫn chưa có do đánh giá chưa kịp hoàn thành.
...
($\beta=0{,}15$, Thí nghiệm 5) đã được dùng để huấn luyện chưng cất MicroDiT trên toàn
bộ dữ liệu tới hết lịch trình dự kiến, với ground-truth loss hội tụ ổn định; tuy nhiên
đánh giá chất lượng sinh khách quan trên đúng checkpoint cuối này chưa kịp hoàn thành, nên
chưa có số liệu chất lượng cuối cùng để đối chiếu với các kết quả sơ bộ ở Thí nghiệm 4.
Do đó, kết luận phù hợp nhất là đề tài đã hoàn thành một pipeline nghiên cứu khả dụng,
minh bạch về giới hạn và có cơ sở thực nghiệm để mở rộng, với bằng chứng hội tụ rõ ràng ở
mức huấn luyện, chứ chưa phải một hệ thống sinh bài hát đã được xác nhận đầy đủ về chất
lượng. Các nút thắt còn lại (độ rõ lời, tính ổn định giai điệu) không có bằng chứng nào
cho thấy là giới hạn cấu trúc của phương pháp; nhiều khả năng đây là vấn đề về quy mô --
dữ liệu huấn luyện, số bước/epoch, hoặc tài nguyên tính toán -- và cần được kiểm chứng
bằng cách mở rộng thêm ở các hướng nêu dưới đây.
...
\begin{enumerate}[label=\arabic*)]
    \item Hoàn thành đánh giá chất lượng sinh khách quan (SD cao độ, tỉ lệ voiced, WER) trên
      checkpoint MicroDiT vừa chưng cất tới hội tụ ($\beta=0{,}15$, ground-truth loss
      $1{,}25\to0{,}46$, \S\ref{sec:exp4_mel_vs_latent}) trên nhiều mẫu hơn một vài bài --
      đây là hành động ưu tiên cao nhất vì huấn luyện đã xong nhưng phần xác nhận chất
      lượng chưa kịp hoàn thành trước khi nộp báo cáo.
