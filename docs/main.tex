...
\end{equation}
một lần duy nhất ở giai đoạn tiền tính (\texttt{precompute\_latent\_dataset.py}), \textbf{trước khi} đưa qua bộ mã hoá latent (VAE, \S\ref{sec:latent_encoder_arch}) đã đóng băng decoder. Bộ mã hoá nén $a^{\mathrm{mix}}$ thành $x_1=\mathrm{Enc}(a^{\mathrm{mix}})\in\mathbb{R}^{F\times T}$ với $F=64$ ở tần số \textbf{5~Hz} -- đúng không gian latent thật mà teacher DiffRhythm2 (không phải mel-spectrogram) sử dụng nội bộ, nên student dùng thẳng biểu diễn này, không cần lớp ánh xạ số chiều hay resample tần số nào giữa hai phía. Mỗi bản ghi huấn luyện chỉ còn một latent cả-bài duy nhất, không còn tách vocal/backing riêng ở bước huấn luyện CFM. Hệ thống cần học trường velocity:
\begin{equation}
...
\textbf{Chú ý quan trọng về vai trò của backing}: $a^{\mathrm{acc}}$ (backing) chỉ được dùng để \emph{xây dựng mục tiêu huấn luyện} $x_1$ -- đã hoà vào latent cả-bài từ trước khi huấn luyện (công thức \eqref{eq:reconstruct_full_mix}) — nó \textbf{không} là một tensor điều kiện đưa vào mạng $v_\theta$. Do đó model phải học sinh ra cả bài (giọng hát và nhạc đệm) từ trạng thái không điều kiện theo backing, chỉ dựa vào ca từ và style embedding toàn cục. Để tránh model "lãng quên" tín hiệu giọng hát (yếu hơn, thưa hơn nhạc đệm) khi tối ưu trực tiếp trên hỗn hợp, một đầu ra phụ trợ dự đoán riêng velocity của vocal (\emph{vocal auxiliary head}) được huấn luyện đồng thời. Đầu phụ trợ này cần một mục tiêu vocal-only riêng biệt với $x_1$; dữ liệu tiền tính hiện chỉ giữ lại latent cả-bài (không có latent vocal-only riêng), nên đầu phụ trợ hiện chưa nhận được tín hiệu vocal-only thật sự khác với mục tiêu chính -- một giới hạn hiện tại của cài đặt, không phải của thiết kế kiến trúc.
...
model sinh. Đo tỉ lệ khung có giọng trên 6 bài khác nhau cho ba giá trị $\alpha$ --
$60{,}7\%$ ở $\alpha=0{,}5$, $\mathbf{92{,}4\%}$ ở $\alpha=0{,}8$, và $77{,}8\%$ ở
$\alpha=0{,}9$ (vocal thật: $74{,}3\%$) -- xác nhận $\alpha\approx0{,}8$ là điểm tối ưu thật
(quan hệ phi tuyến, không phải "$\alpha$ cao hơn luôn tốt hơn"), nên $\alpha=0{,}8$ được
dùng làm mặc định thực tế. Nếu teacher hoặc tokenizer không tải được, $\alpha$ bị ép về
$1{,}0$ (chỉ còn $\mathcal{L}_{\mathrm{gt}}$) và trạng thái này được báo cáo rõ ràng trong
log, không âm thầm.
...
Thực nghiệm được thiết kế theo ba tầng. Tầng thứ nhất xác nhận tính đúng của biểu diễn âm thanh và vocoder. Tầng thứ hai so sánh không gian biểu diễn của student (mel 100-chiều/93,75~Hz so với latent thật 64-chiều/5~Hz của teacher). Tầng thứ ba kiểm tra bottleneck xác suất của bộ mã hoá latent. Cách tổ chức theo tầng giúp kết luận tương ứng với bằng chứng, thay vì dùng một file âm thanh đầu ra để suy diễn cho mọi thành phần.
...
\begin{description}
 ...
 \item[RQ2:] Cho student không gian latent thật của teacher (thay vì mel) có thay đổi được vấn đề "âm thanh sinh ra nghe như nhiễu" đã lặp lại ở các thí nghiệm mel-space trước đó không, và nguyên nhân của sự khác biệt (nếu có) là gì?
 \item[RQ3:] Bottleneck xác suất (VAE, mu/logvar + KL divergence) có khắc phục được hiện tượng collapse của latent encoder ở quy mô dữ liệu lớn hay không, và trọng số $\beta$ nào là tối ưu?
\end{description}

\section{Nguồn dữ liệu}

Báo cáo chỉ sử dụng các giá trị có trong log, artifact hoặc checkpoint của dự án. Dữ liệu đánh giá gồm hai nhóm chính. Nhóm kiểm tra vocoder dùng các đoạn âm thanh thật để thực hiện vòng lặp waveform $\rightarrow$ mel $\rightarrow$ waveform. Nhóm huấn luyện latent-space dùng 248 bản ghi (chưng cất MicroDiT, Thí nghiệm 2) và toàn bộ 1839 bài (ablation bộ mã hoá latent, Thí nghiệm 3). Một tập ứng viên lớn hơn có 201 bài, dung lượng khoảng 2,52~GB, được dùng để ước lượng chi phí tiền xử lý chứ chưa được dùng để công bố kết quả huấn luyện hoàn chỉnh.

Cần phân biệt reconstruction và generation. Trong reconstruction, mel được trích từ chính waveform tham chiếu nên correlation và RMSE có cặp mẫu tương ứng. Trong generation, waveform mới không có waveform đích duy nhất; do đó correlation với bài gốc không phải thước đo chất lượng sinh hợp lệ.
...
Đề tài đã nghiên cứu và xây dựng thành công pipeline sinh ca khúc tiếng Việt hoàn chỉnh từ lời hát và mô tả phong cách, bao phủ trọn vẹn quy trình từ xử lý dữ liệu, trích xuất đặc trưng đến chưng cất tri thức và suy luận. Từ các kết quả thực nghiệm, đề tài đúc kết được ba đóng góp và phát hiện cốt lõi:
