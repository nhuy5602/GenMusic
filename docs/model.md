...
`src/training/distill_training.py` replicates the *real* DiffRhythm2 teacher's call contract. However, there is a known **structural and temporal mismatch** in the current chưng cất bridge:
- **Data Space Mismatch**: The teacher model actually operates on **64-dimensional VAE latents** generated by its customized 5 Hz Stable Audio 2 VAE, not on a 64-channel Mel-spectrogram.
- **Temporal Mismatch**: The teacher VAE operates at a frame rate of **5 Hz** (5 frames per second), whereas the student operates on Mel-spectrograms at **93.75 Hz** (93.75 frames per second).
- **Current Bridge Limitation**: The current adapter (`_resize_mel_bins`) only resizes the frequency dimension (100 -> 64 bins) but does not compress the time dimension. As a result, a 93.75 Hz sequence (e.g., 375 frames for 4s) is fed directly into the teacher which was trained on 5 Hz sequences (20 frames for 4s), causing the teacher's position embeddings and attention mask to run out-of-distribution.
- A small trainable linear adapter (`from_teacher_mel`) bridges the teacher's 64-dim output back to the student's 100-mel space for distillation loss calculation. 

If the teacher (or its lyric tokenizer) can't be loaded — no internet, DiffRhythm2 repo not
