...
def submit_text_to_music_job(
    *,
    ...
    if not readiness["ready"]:
        ...
        state["messages"].append("Hãy cài/cấu hình Kaggle API rồi chạy lại các lệnh đã tạo.")
        _write_state(state)
        return state
...
def submit_tts_retry_job(
    ...
    *,
    ...
    if not config.submit:
        state["status"] = "staged"
        state["messages"].append("Job thử lại TTS đã được chuẩn bị local. Tắt bước submit.")
        _write_state(state)
        return state
    ...
    if not readiness["ready"]:
        state["status"] = "needs_setup"
        state["messages"].append("Hãy cài/cấu hình Kaggle API rồi thử lại TTS.")
        _write_state(state)
        return state
        ...
        json.dumps(
            {
                ...
                "id": dataset_ref,
                "licenses": [{"name": "other"}],
                "subtitle": "Request văn bản tiếng Việt và source pipeline để sinh nhạc trên Kaggle.",
...
def stage_tts_retry_job(parent_state: dict[str, Any], *, config: KaggleJobConfig) -> dict[str, Any]:
        ...
        json.dumps(
            {
                ...
                "id": dataset_ref,
                "licenses": [{"name": "other"}],
                "subtitle": "Dataset thử lại chỉ TTS cho GenMusic VN.",
    ...
    state = {
        ...
        "vocal_path": "",
        "vocal_url": "",
        "custom_model_failed": bool(parent_state.get("custom_model_failed")),
        ...
        "messages": [
            ...
            "The previous backing MP3 will be reused; the custom composer will not run again.",
        ],
        "last_error": "",
        "custom_model_error": parent_state.get("custom_model_error", ""),
...
def submit_kaggle_job(
    ...
    *,
    ...
    state["messages"].append("Đã upload request văn bản và source lên Kaggle.")
    _write_state(state)
    ...
    if pushed["returncode"] != 0:
        ...
        state["messages"].append("Submit kernel thất bại.")
        _mark_terminal(state, "failed")
        _write_state(state)
        return state
    ...
    if wait:
        ...
        state["messages"].append("Hết thời gian chờ job Kaggle.")
...
def refresh_kaggle_job(state_or_path: dict[str, Any] | str | Path) -> dict[str, Any]:
    ...
    if status["returncode"] != 0:
        ...
        state["messages"].append("Không thể đọc trạng thái kernel Kaggle.")
        _mark_terminal(state, "failed")
        _write_state(state)
        return state
...
def sync_kaggle_artifact(*, source: str, ref: str, output_dir: str | Path = "models/current") -> dict[str, Any]:
    cli = kaggle_cli_command()
    if cli is None:
        raise KaggleAutoError("Không tìm thấy Kaggle CLI. Cài bằng `pip install kaggle`.")
    ...
    if source == "dataset":
        ...
        raise KaggleAutoError("Source không được hỗ trợ. Dùng `dataset` hoặc `kernel`.")
    ...
    if result["returncode"] != 0:
        raise KaggleAutoError(result["stderr"] or result["stdout"] or "Đồng bộ Kaggle thất bại.")
    return manifest
...
def kaggle_readiness(username: str | None = None) -> dict[str, Any]:
    ...
    cli = kaggle_cli_command()
    if cli is None:
        messages.append("Thiếu Kaggle CLI. Cài bằng `pip install kaggle`.")

    user = resolve_kaggle_username(username)
    if not user:
        messages.append("Thiếu username Kaggle. Đặt KAGGLE_USERNAME trong .env hoặc environment.")

    tokens = load_kaggle_api_tokens()
    has_key = bool(tokens.get("KAGGLE_KEY"))
    if not has_key:
        messages.append("Thiếu API key Kaggle. Đặt KAGGLE_KEY trong .env hoặc environment.")
...
def build_duration_plan(request: dict, result) -> dict:
    ...
    return {{
        ...
        "custom_render_duration_seconds": planned,
...
def render_mms_tts_vocal(request: dict, result, duration_plan: dict) -> Path:
    ...
    # Keep MMS TTS on CPU so the fallback path stays predictable on Kaggle.
    device = "cpu"
...
def render_custom_backing_mp3(request: dict, result, duration_plan: dict) -> Path:
    from genmusic_vn.core.generators.base import GeneratorInput
    from genmusic_vn.core.generators.guide_track import GuideTrackGenerator
    ...
    generator_input = GeneratorInput(
        ...
        duration_seconds=int(duration_plan.get("planned_backing_duration_seconds", request.get("duration_seconds", 30))),
    )
    custom_dir = OUTPUT_DIR / "custom_model"
    generated = GuideTrackGenerator().generate(generator_input, custom_dir)
    wav_path = custom_dir / "backing.wav"
    midi_path = custom_dir / "song.mid"
    if midi_path.exists():
        shutil.copy2(midi_path, OUTPUT_DIR / f"{{request.get('run_id', 'genmusic_vn')}}_song.mid")
    if wav_path.exists():
        shutil.copy2(wav_path, OUTPUT_DIR / f"{{request.get('run_id', 'genmusic_vn')}}_backing.wav")
    mp3_candidate = custom_dir / "final.mp3"
    mp3_path = OUTPUT_DIR / f"{{request.get('run_id', 'genmusic_vn')}}_backing.mp3"
    if mp3_candidate.exists():
        shutil.copy2(mp3_candidate, mp3_path)
        return mp3_path
...
def main() -> None:
    ...
    from genmusic_vn.core.schemas import to_plain_data
    ...
    final = {{
        ...
        "custom_model_failed": bool(custom_model_error),
        "musicgen_failed": False,
...
def _wait_for_dataset_ready(
    ...
    *,
    ...
    _append_message_once(state, "Đang chờ dataset Kaggle sẵn sàng.")
    _write_state(state)
    ...
    while time.time() < deadline:
        ...
        if result["returncode"] != 0:
            ...
            if "403" in error or "GetDatasetStatus" in error:
                ...
                _append_message_once(
                    state,
                    "Không đọc được trạng thái dataset Kaggle; tiếp tục vì upload đã thành công.",
                )
                _write_state(state)
                return True
            state["status"] = "failed"
            state["last_error"] = error
            _append_message_once(state, "Không thể đọc trạng thái dataset Kaggle.")
            return False
        ...
        if status_text in {"ready", "complete", "completed"}:
            state["status"] = "dataset_ready"
            _set_timestamp(state, "dataset_ready_at")
            _append_message_once(state, "Dataset Kaggle đã sẵn sàng.")
            _write_state(state)
            return True
        if status_text in {"error", "failed", "cancelled", "canceled"}:
            state["status"] = "failed"
            state["last_error"] = result["stdout"] or "Xử lý dataset Kaggle thất bại."
            _append_message_once(state, "Xử lý dataset Kaggle thất bại.")
            _mark_terminal(state, "failed")
            return False
    ...
    state["status"] = "timeout"
    state["last_error"] = "Hết thời gian chờ dataset Kaggle sẵn sàng."
...
def _download_kernel_output(state: dict[str, Any], cli: list[str], *, expect_mp3: bool) -> None:
    ...
    if output_error and not files:
        ...
        _append_message_once(state, "Tải output Kaggle thất bại.")
        _mark_terminal(state, "failed")
        return
    ...
    if mp3_files:
        _remove_messages(
            state,
            "Tải output Kaggle thất bại.",
            "Đã tải output Kaggle nhưng không tìm thấy file MP3.",
        )
        ...
        if "f5_tts_vocal_mix" in backend:
            ...
            if "f5_failed" in backend:
                _append_message_once(state, "F5-TTS lỗi trên Kaggle; MMS Vietnamese TTS dự phòng đã được mix vào MP3 cuối.")
            else:
            ...
            _append_message_once(state, "Đã tải MP3 output Kaggle về máy local.")
        if output_error:
        ...
        if output_error and not state.get("last_error"):
            state["last_error"] = output_error
        _append_message_once(state, "Đã tải output Kaggle nhưng không tìm thấy file MP3.")
        _mark_terminal(state, "failed")
    else:
        if output_error and not state.get("last_error"):
            state["last_error"] = output_error
        _append_message_once(state, "Đã tải log lỗi Kaggle.")
        _mark_terminal(state, "failed")
    ...
    for path in files:
        ...
        if isinstance(duration_plan, dict):
            ...
            for key in (
                ...
                "custom_render_duration_seconds",
        ...
        custom_model_failed = data.get("custom_model_failed")
        if isinstance(custom_model_failed, bool):
            state["custom_model_failed"] = custom_model_failed
        ...
        custom_model_error = data.get("custom_model_error")
        if isinstance(custom_model_error, str) and custom_model_error.strip():
            state["custom_model_error"] = custom_model_error.strip()[-1000:]
            state["last_error"] = custom_model_error.strip()[-1000:]
