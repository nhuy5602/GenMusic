...
def _wait_for_dataset_visible(
    ...
    *,
    ...
    """Wait for a ready dataset or an indexed immutable marker file."""
    for _ in range(180):
        status_result = run_cli(
            cli,
            ["datasets", "status", dataset_ref],
            env,
            timeout=120,
        )
        status = (
        ...
        if (
            ...
            and expected_marker in files_text
        ):
            time.sleep(15)
            return
        time.sleep(5)
    raise TimeoutError(
        f"Kaggle dataset did not become visible: {dataset_ref}"
    )
...
def _create_dataset(
    *,
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"Could not create Kaggle dataset {dataset_ref}"
        )
...
def submit_phase_kernel(
    ...
    *,
    ...
    result = run_cli(context.cli, push_args, context.environment, timeout=900)
...
def new_run_dir(context: SubmitContext, phase: str) -> tuple[int, Path]:
    ...
    while True:
        ...
        try:
            # Two independent phase submissions can start within one second.
            # Claim the directory atomically so they never overwrite each
            # other's kernel metadata or state.json.
