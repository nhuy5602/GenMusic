from scripts.run_colab_master_raw_connected_pacing_v80 import (
    LINE_BREAK_PAUSE_CAP,
    SAME_LINE_PAUSE_CAP,
    connected_continuity_gate,
    fill_connected_line_gaps,
)
from scripts.run_colab_master_waveform_unit_v64 import Unit
from scripts.run_kaggle_master_raw_connected_pacing_v80 import (
...
def test_v80_kaggle_bundle_is_goal_eligible_and_token_free() -> None:
    ...
    assert (
        "scripts/run_colab_master_raw_connected_pacing_v80.py"
        in PATCH_FILES
    )
    code = _kernel_code(
        patch_sha256="A" * 64,
    ...
    assert "STARTING_MASTER_RAW_LINE_PACING_V80" in code
    assert (
        "scripts/run_colab_master_raw_connected_pacing_v80.py"
        in code
    )
    ...
    assert (
        "'scripts/run_colab_master_raw_line_pacing_v79.py'"
        in code
    )
