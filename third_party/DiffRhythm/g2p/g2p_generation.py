...
def get_segment(text: str) -> List[str]:
    ...
    segments.append((temp_seg, temp_lang))
    return [(_segment_text, _latin_language(_segment_text) if _segment_lang == "en" else _segment_lang) for _segment_text, _segment_lang in segments]


def _latin_language(text: str) -> str:
    """Detect Vietnamese diacritics before the upstream English fallback."""

    vietnamese_letters = set("ăâđêôơưĂÂĐÊÔƠƯ")
    if any(character in vietnamese_letters for character in text):
        return "vi"
    return "en"
