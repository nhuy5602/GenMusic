...
SOURCE_ZIP_EXCLUDED_DIRS = {
    ...
    "ckpt",
    "DiffRhythm2-main",
    # Downloaded/licensed corpora are uploaded as explicit data assets with
    # their own provenance hash.  Bundling them again as source duplicates
    # hundreds of MB and can silently mix research-only data into unrelated
    # kernels.
    "external_sources",
    "local_notes",
    ".genmusic",
...
def write_source_zip(project_root: Path, destination: Path) -> None:
    ...
    else:  # pragma: no cover - fallback for source exports without .git
        candidates = list(project_root.rglob("*"))
    with zipfile.ZipFile(destination, "w", zipfile.ZIP_DEFLATED) as archive:
        for path in sorted(candidates):
            relative = path.relative_to(project_root)
            if not path.is_file() or any(
                part in SOURCE_ZIP_EXCLUDED_DIRS for part in relative.parts
            ):
                continue
...
def run_local_generation(
    *,
    ...
    if backing_mel:
        backing_condition = _load_generation_condition(
            backing_mel, keys=("mel", "backing_mel")
        )
    if style_anchor:
    ...
    if checkpoint and Path(checkpoint).exists():
        model, config, payload = load_checkpoint(
            checkpoint, device=selected_device, roberta_model=roberta_model
        )
    ...
    if effective_duration > requested_duration:
        print(
            f"[INFO] Duration tang tu {requested_duration:.2f}s len {effective_duration:.2f}s de tranh ep lyric.",
            flush=True,
        )
    ...
    report.update(
        {
            ...
            "backing_mel_path": str(Path(backing_mel).resolve())
            if backing_mel
            else None,
            "style_anchor_path": str(Path(style_anchor).resolve())
            if style_anchor
            else None,
        }
    )
...
def stage_text_to_music_job(
    *,
    ...
    requested_duration = min(
        120, max(requested_duration, math.ceil(estimate_minimum_lyric_duration(text)))
    )
    ...
    training_dataset_ref = resolve_training_dataset_ref(
        config.training_dataset_ref, config.username
    )
    ...
    style_prompt = (
        genre or "Vietnamese pop ballad, warm piano, emotional strings, clear melody"
    )
    request = {
        "run_id": run_id,
        "text": normalized,
    ...
    (dataset_dir / "dataset-metadata.json").write_text(
        json.dumps(
            {
                "title": request_dataset_slug,
                "id": request_dataset_ref,
                "licenses": [{"name": "other"}],
                "subtitle": "Request cho model GenMusic tự code.",
                "description": "Private request dataset for the self-authored GenMusic diffusion model.",
            },
            ensure_ascii=False,
            indent=2,
        ),
    ...
    (kernel_dir / "run_genmusic.py").write_text(
        _kernel_script(request_dataset_slug, training_dataset_slug), encoding="utf-8"
    )
        ...
        json.dumps(
            {
                ...
                "dataset_sources": [training_dataset_ref, request_dataset_ref],
                "competition_sources": [],
                "kernel_sources": [],
                "model_sources": [],
            },
    ...
    state = {
        ...
        "commands": commands,
        "messages": [
            f"Dataset training cố định: {training_dataset_ref}.",
            "Đã đóng gói request và source model tự code; chưa submit Kaggle.",
        ],
        "history": [],
...
def submit_text_to_music_job(
    *,
    ...
    if not kaggle_dataset_exists(state["training_dataset_ref"]):
        return _fail(
            state,
            f"Không tìm thấy dataset training Kaggle '{state['training_dataset_ref']}'. Hãy chạy make-and-upload-dataset trước.",
        )
...
def submit_kaggle_job(
    state: dict[str, Any], *, wait: bool, poll_seconds: int, timeout_seconds: int
) -> dict[str, Any]:
    cli = kaggle_cli_command()
    if cli is None:
        return _fail(state, "Không tìm thấy Kaggle CLI.")
    created = _run(
        cli + ["datasets", "create", "-p", state["dataset_dir"], "-r", "zip"],
        timeout=900,
    )
    ...
    if created["returncode"] != 0:
        ...
        if not _wait_for_dataset_ready(
            cli, state["request_dataset_ref"], timeout_seconds=900
        ):
            verified = _run(
                cli + ["datasets", "status", state["request_dataset_ref"]], timeout=120
            )
            state["history"].append(_history_item("datasets status", verified))
            return _fail(state, _summarize_cli_error(created))
        state["messages"].append(
            "Kaggle đã nhận dataset dù CLI báo lỗi parse phản hồi; đã xác nhận trạng thái ready."
        )
    elif not _wait_for_dataset_ready(
        cli, state["request_dataset_ref"], timeout_seconds=900
    ):
        return _fail(
            state,
            f"Dataset request Kaggle '{state['request_dataset_ref']}' chưa chuyển sang trạng thái ready.",
        )
    ...
    state["messages"].append(
        "Kernel Kaggle đã được submit; có thể mở link để xem tiến trình."
    )
    state["last_error"] = ""
...
def refresh_kaggle_job(state_or_path: dict[str, Any] | str | Path) -> dict[str, Any]:
    state = (
        state_or_path if isinstance(state_or_path, dict) else _load_state(state_or_path)
    )
    cli = kaggle_cli_command()
    if cli is None:
        return _fail(state, "Không tìm thấy Kaggle CLI.")
    ...
    if any(value in status_text for value in ("complete", "completed", "succeeded")):
        ...
        state["downloaded_files"] = [
            str(path)
            for path in Path(state["download_dir"]).rglob("*")
            if path.is_file()
        ]
        _attach_artifact_urls(state)
    elif any(
        value in status_text for value in ("error", "failed", "cancelled", "canceled")
    ):
        state["status"] = "failed"
...
def kaggle_readiness(username: str | None = None) -> dict[str, Any]:
    ...
    ready = bool(
        tokens.get("KAGGLE_USERNAME")
        and kaggle_auth_available(tokens)
        and kaggle_cli_command()
    )
    if not ready:
        return {
            "ready": False,
            "messages": [
                "Cần KAGGLE_USERNAME, KAGGLE_API_TOKEN=KGAT_... (hoặc KAGGLE_KEY cũ) và Kaggle CLI."
            ],
        }
...
def _wait_for_dataset_ready(
    cli: list[str], dataset_ref: str, *, timeout_seconds: int
) -> bool:
    ...
    while time.time() < deadline:
        ...
        if result["returncode"] == 0 and any(
            value in status_text
            for value in ("failed", "deleted", "cancelled", "canceled")
        ):
            return False
        time.sleep(5)
    return False
...
def _dataset_status_is_ready(result: dict[str, Any]) -> bool:
    ...
    return (
        status_text == "ready"
        or 'status "ready"' in status_text
        or status_text.endswith("status ready")
    )
...
def upload_dataset_to_kaggle(
    ...
    *,
    ...
    if cli is None or not resolved_username or not kaggle_auth_available():
        ...
        (root / "kaggle_upload_report.json").write_text(
            json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8"
        )
        return report
    if dataset_ref:
    ...
    metadata_path.write_text(
        json.dumps(
            {
                "title": "GenMusic VN self-diffusion training",
                "id": dataset_ref,
                "licenses": [{"name": "other"}],
                "subtitle": "Synthetic mel dataset for the self-authored text-to-music model.",
                "description": "Synthetic structured mel tensors and Vietnamese text/style conditions for pipeline training smoke tests.",
            },
            ensure_ascii=False,
            indent=2,
        ),
    ...
    result = _run(
        cli + ["datasets", "create", "-p", str(root), "-r", "zip"],
        timeout=timeout_seconds,
    )
    dataset_ready = result["returncode"] == 0 and _wait_for_dataset_ready(
        cli, dataset_ref, timeout_seconds=min(timeout_seconds, 900)
    )
    status = (
        "uploaded"
        if dataset_ready
        else ("pending" if result["returncode"] == 0 else "failed")
    )
    report = {
        "status": status,
        "dataset": str(root),
        "dataset_ref": dataset_ref,
        "dataset_url": _dataset_url(dataset_ref),
...
def _replace_auth_credentials(
    values: dict[str, str], preferred: dict[str, str]
) -> None:
...
def load_kaggle_api_tokens() -> dict[str, str]:
    ...
    config_dir = Path(
        os.getenv("KAGGLE_CONFIG_DIR") or Path.home() / ".kaggle"
    ).expanduser()
    ...
    if not values.get(KAGGLE_API_TOKEN_ENV) and values.get(
        KAGGLE_ACCESS_TOKEN_ALIAS_ENV
    ):
    ...
    if not values.get(KAGGLE_API_TOKEN_ENV) and key_value.startswith(
        ("KGAT_", "KGAT-")
    ):
...
def validate_dataset_ref(dataset_ref: str) -> str:
    ...
    if len(parts) != 2 or not all(parts) or any(char.isspace() for char in value):
        raise ValueError(
            "Dataset ref phải có dạng owner/slug, ví dụ user/genmusic-vn-self-diffusion-training."
        )
    return value


def resolve_training_dataset_ref(
    dataset_ref: str | None = None, username: str | None = None
) -> str:
    configured = (
        dataset_ref
        or os.getenv(KAGGLE_DATASET_ENV)
        or load_kaggle_api_tokens().get(KAGGLE_DATASET_ENV)
    )
    if configured:
...
def _dataset_url(dataset_ref: str) -> str:
    return (
        ...
        if not dataset_ref.startswith("YOUR_KAGGLE_USERNAME/")
        else ""
    )
...
def _make_lrc(text: str, duration: int):
    ...
    return [
        AlignedLine(line, index * span, (index + 1) * span, "generated")
...
def _write_source_zip(destination: Path) -> None:
    write_source_zip(PROJECT_ROOT, destination)


def _commands(
    dataset_dir: Path, kernel_dir: Path, download_dir: Path, kernel_ref: str
) -> list[str]:
    return [
...
def _convert_to_mp3(wav_path: Path) -> Path | None:
    ...
    result = subprocess.run(
        [ffmpeg, "-y", "-i", str(wav_path), str(destination)],
        capture_output=True,
        text=True,
        check=False,
    )
...
def _run(command: list[str], *, timeout: int) -> dict[str, Any]:
    try:
        result = subprocess.run(
            ...
            text=True,
            timeout=timeout,
            env={**os.environ, **load_kaggle_api_tokens()},
        )
        return {
            "command": command,
            "returncode": result.returncode,
...
def _summarize_cli_error(result: dict[str, Any]) -> str:
    return (result.get("stderr") or result.get("stdout") or "Kaggle command failed")[
        -2000:
    ]
...
def _attach_artifact_urls(state: dict[str, Any]) -> None:
    ...
    if lrc.exists():
        try:
            state["lrc_url"] = (
                "/outputs/"
                + lrc.resolve().relative_to(output_root.resolve()).as_posix()
            )
        except ValueError:
            pass
