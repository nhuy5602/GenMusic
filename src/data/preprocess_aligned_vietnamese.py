...
def preprocess_aligned_parquet(
    ...
    *,
    ...
    """Separate stems and preserve exact lyric timestamps.

    ``raw_audio=True`` keeps the pristine separated 24 kHz waveform tensors
    instead of converting them to mel tensors.  This is the representation
    required by waveform-unit experiments; no vocoder round-trip is involved.
    """
...
def main() -> None:
    ...
    parser.add_argument(
        "--raw-audio",
        action="store_true",
        help="Save separated 24 kHz waveform tensors instead of mels.",
    )
