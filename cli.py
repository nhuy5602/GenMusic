...
def build_parser() -> argparse.ArgumentParser:
    ...
    train.add_argument("--minimum-epochs", type=int, default=8, help="Early stopping never fires before this many epochs, regardless of --epochs. A checkpoint only ever counts as 'improved' once text_conditioning_sensitivity clears --minimum-text-sensitivity, so if that gate never opens, training stops at exactly this many epochs -- raise it to force more optimization steps before giving up.")
    train.add_argument("--early-stopping-patience", type=int, default=4, help="Epochs without an improved (validation-loss-AND-sensitivity-gate) checkpoint after --minimum-epochs before stopping.")
    train.add_argument("--minimum-text-sensitivity", type=float, default=None, help="Overrides the text_sensitivity_target-derived floor a checkpoint's text_conditioning_sensitivity must clear to count as an improvement (see _is_checkpoint_improvement). Default (None) keeps train_model's own default floor.")
    ...
    distill.add_argument("--log-every-steps", type=int, default=100, help="Print a running loss every N steps -- distillation used to only log once per epoch, indistinguishable from a hang on a large dataset.")
    distill.add_argument("--resume-checkpoint", default=None, help="Load a prior run's student weights before training starts (fresh optimizer/schedule) -- e.g. to keep pushing a checkpoint for a few more bounded epochs instead of retraining from scratch.")
    distill.add_argument("--num-workers", type=int, default=2, help="DataLoader background workers to prefetch batches while the GPU computes.")
    distill.add_argument("--amp", action="store_true", help="Use fp16 automatic mixed precision for the student forward/backward and the frozen teacher's forward pass -- real speedup on GPUs with fp16 tensor cores (e.g. T4), no effect on CPU.")
