...
## Pipeline JAM/DiffRhythm

Project hiện có thêm pipeline sinh nhạc từ văn bản theo hướng conditional flow matching (CFM) + Diffusion Transformer (DiT). Pipeline này là đường huấn luyện mới; backend custom cũ vẫn được giữ để demo nhanh và tương thích ngược trong lúc chưa có checkpoint DiT đầy đủ.

### 1. Tiền xử lý dữ liệu

Manifest nguồn JSON/JSONL phải chỉ rõ `id`, `audio` và `lyrics` (inline hoặc đường dẫn). Dataset chỉ có MP3 có thể dùng `--asr-model` để bóc transcript bằng faster-whisper. Lyrics được chuẩn hóa số, viết tắt, Unicode và section; G2P tạo token IPA kèm tone `1..6`; alignment tạo LRC từ ASR segment. Không có timestamp thật thì pipeline không tự ghi giả, trừ khi bật `--allow-heuristic` cho smoke test.

```powershell
python -m genmusic_vn.cli normalize-lyrics --input data/song.txt --out outputs/normalized.txt
python -m genmusic_vn.cli lyrics-g2p --input outputs/normalized.txt --out outputs/g2p.json --no-phonemizer
python -m genmusic_vn.cli align-lyrics --audio data/song.wav --lyrics outputs/normalized.txt --segments data/song_segments.json --out outputs/song.lrc
python -m genmusic_vn.cli prepare-jam-dataset --manifest data/source_manifest.jsonl --audio-root data --out datasets/processed/jam --asr-model medium --vae-checkpoint checkpoints/vae.ts --style-checkpoint checkpoints/style.ts --max-files 0
...
`prepare-jam-dataset` yêu cầu VAE và style encoder thật để dùng cho training. `--allow-proxy` chỉ tạo tensor đúng shape để kiểm tra pipeline, mọi artifact proxy đều được đánh dấu `proxy-not-for-training` và không được dùng trong báo cáo học thuật.

### 2. SFT, DPO, chưng cất và INT8

```powershell
python -m genmusic_vn.cli train-jam-sft --manifest datasets/processed/jam/manifest.jsonl --out outputs/jam_sft_demo --preset demo --steps 100
python -m genmusic_vn.cli train-jam-sft --manifest datasets/processed/jam/manifest.jsonl --out outputs/jam_sft --preset diffrhythm --steps 100000 --device cuda
python -m genmusic_vn.cli build-jam-dpo-pairs --scores outputs/candidates_f0.jsonl --out datasets/processed/dpo_pairs.jsonl
python -m genmusic_vn.cli train-jam-dpo --checkpoint outputs/jam_sft/sft_checkpoint.pt --pairs datasets/processed/dpo_pairs.jsonl --out outputs/jam_dpo --device cuda
python -m genmusic_vn.cli distill-jam --teacher outputs/jam_dpo/dpo_checkpoint.pt --manifest datasets/processed/jam/manifest.jsonl --out outputs/jam_student --teacher-steps 32 --student-steps 4 --device cuda
python -m genmusic_vn.cli quantize-jam --checkpoint outputs/jam_dpo/dpo_checkpoint.pt --out models/current/jam_diffrhythm_int8.pt
...
Stage consistency distillation hiện được cung cấp ở `genmusic_vn/training/distillation.py` dưới dạng helper để tích hợp teacher sampler 32--100 bước và student sampler 4 bước. Cần chạy distillation trên checkpoint DiT thực tế; không có shortcut nào biến custom symbolic/audio proxy thành model hát chất lượng phòng thu.

### 3. Đánh giá và plot

```powershell
python -m genmusic_vn.cli evaluate-jam --generated outputs/generated.wav --reference data/reference.wav --generated-text outputs/asr.txt --reference-text data/lyrics.txt --votes evaluation/double_blind.csv --out outputs/jam_evaluation
```

Báo cáo tạo `evaluation_report.json` và `plots/` gồm FAD, MCD, FFE, WER, MOS, CMOS. Metric để `null` khi thiếu embedding/F0/transcript tương ứng, không thay bằng số giả. Phiếu MOS/CMOS phải có tối thiểu 15 người nghe bản địa; cột gợi ý là `listener_id, model_variant, musicality, intelligibility, comparison`.

### 4. Giới hạn cần ghi trong báo cáo

Tên “JAM/DiffRhythm” ở đây mô tả hướng kiến trúc và pipeline của project. Checkpoint, VAE, style encoder, dữ liệu lyric và dữ liệu audio phải được cung cấp riêng với license phù hợp. Thông số 402M trong đề cương chưa được ghi cứng: tài liệu công khai của JAM/DiffRhythm có các biến thể kích thước khác nhau, vì vậy report luôn ghi `parameter_count` thực tế từ checkpoint/config.
