...
genmusic_vn/
  ...
  data/               # tạo, nhập và quản lý dataset
    ...
    reference_dataset.py # dataset tham chiếu an toàn
...
Output của crawler có thể truyền vào `--extra-dataset` khi nhãn emotion/style hợp lệ. Các `rhyme_features` được dùng để học vần cuối và điệp âm ở cấp verse/chorus. Profile đã học chỉ là gợi ý, không ép vần khi câu trở nên thiếu tự nhiên.
